...
#endif
{
  ...
  Value *origptr = ptr;
  DebugLoc dbg = getNewFromOriginal(orig->getDebugLoc());
  ...
  auto rule = [&, align, isVolatile, ordering, syncScope, orig,
               dbg](IRBuilder<> &BuilderM, Value *ptr, Value *newval) {
    ...
    //    for (ssize_t j = -1; j < getWidth(); j++) { // TODO: fix this!
    //      if (j != (ssize_t)idx)
  ...
  auto rule_mask = [M, align, orig, &idx, dbg](IRBuilder<> &BuilderM,
                                               Value *ptr, Value *newval,
                                               Value *mask) {
    Type *tys[] = {newval->getType(), ptr->getType()};
    auto F = Intrinsic::getDeclaration(M, Intrinsic::masked_store, tys);
    assert(align);
#if LLVM_VERSION_MAJOR >= 10
    Value *alignv =
  ...
  if (!mask) {
    ...
    applyChainRule(BuilderM, rule_mask, Gradient(ptr), Gradient(newval),
                   Primal(mask));
  }
}
...
#endif
{
  ...
  if (start != 0 || needsCast) {
    auto rule = [start, needsCast, &addingType](IRBuilder<> &BuilderM,
                                                Value *ptr) {
      if (start != 0) {
  ...
  Value *Al1 = A.CreateAlloca(ST);

  if (start != 0 || needsCast) {
    auto rule1 = [ST, start, size, addingType](IRBuilder<> &BuilderM,
                                               Value *dif, Value *Al) {
      if (start != 0) {
        BuilderM.CreateStore(dif,
                             BuilderM.CreatePointerCast(
                                 Al, PointerType::getUnqual(dif->getType())));
        Value *idxs[] = {BuilderM.getInt64(0), BuilderM.getInt32(1)};
    ...
    auto rule2 = [addingType, size, &DL](IRBuilder<> &BuilderM, Value *dif,
                                         Value *Al) {
  ...
  if (Atomic) {
    ...
    if (Arch == Triple::amdgcn &&
      ...
      auto rule = [addingType](IRBuilder<> &BuilderM, Value *ptr) {
    ...
    */
    ...
    if (auto vt = dyn_cast<VectorType>(addingType)) {
...
#endif
      auto rule = [numElems, vt, addingType, op, start,
                   align](IRBuilder<> &BuilderM, Value *dif, Value *ptr) {
      ...
      auto rule = [align, op, start](IRBuilder<> &BuilderM, Value *dif,
                                     Value *ptr) {
#if LLVM_VERSION_MAJOR >= 13
        MaybeAlign alignv = align;
        if (alignv) {
  ...
  if (!mask) {
    ...
    Metadata *scopeMD[1] = {
        getDerivativeAliasScope(origptr, idx)}; // TODO: fix this!!!!
    auto scope = MDNode::get(ptr->getContext(), scopeMD);
    auto dbg = getNewFromOriginal(orig->getDebugLoc());

    auto rule = [addingType, start, size, align, &idx, &scope, dbg, orig,
                 &DL](IRBuilder<> &BuilderM, Value *ptr, Value *dif) {
#if LLVM_VERSION_MAJOR > 7
      ...
      //        if (j != (ssize_t)idx) // TODO: fix this
      //          MDs.push_back(getDerivativeAliasScope(origptr, j));
      //      }
    ...
    Constant *alignv = BuilderM.getInt32(aligni);

    auto rule = [alignv, LF, SF](IRBuilder<> &BuilderM, Value *ptr, Value *dif,
                                 Value *mask) {
    ...
    applyChainRule(BuilderM, rule, Gradient(ptr), Gradient(dif), Primal(mask));
  }
}
  ...
  if (isa<Argument>(val) || isa<Constant>(val)) {
    ...
    if (phi->getNumIncomingValues() == 0) {
      ...
      if (auto dli = dyn_cast_or_null<LoadInst>(hasUninverted(phi))) {
        ...
        auto dbg = getNewFromOriginal(dli->getDebugLoc());

        auto rule = [dli, phi, dbg, &unwrappedLoads = this->unwrappedLoads](
                        IRBuilder<> &BuilderM, Value *pidx) {
#if LLVM_VERSION_MAJOR > 7
          ...
          for (auto B : origLI->getBlocks()) {
            ...
            if (B == origLI->getHeader()) {
              ...
              for (auto inst : loopShadowZeroInits) {
                ...
                auto rule = [funcName, &TLI = this->TLI](
                                IRBuilder<> &NB, Value *inst, Value *anti,
                                ArrayRef<Value *> args) {
                  zeroKnownAllocation(
                      NB, anti, args, funcName, TLI,
                      dyn_cast<CallInst>(inst)); // maybe come up with a
              ...
              if (loopShadowRematerializations.count(&I)) {
                ...
                if (auto orig = dyn_cast<CallInst>(&I)) {
                  ...
                  if (shadowHandlers.find(funcName.str()) !=
                    ...
                    auto dbg = getNewFromOriginal(I.getDebugLoc());

                    auto rule = [orig, dbg](IRBuilder<> &NB, Value *op,
                                            ArrayRef<Value *> args) {
#if LLVM_VERSION_MAJOR >= 11
                      ...
                      cast<CallInst>(anti)->setDebugLoc(dbg);
                    ...
                    if (auto MD = hasMetadata(orig, "enzyme_fromstack")) {
                      ...
                      auto rule1 =
                          [MD, dbg = getNewFromOriginal(I.getDebugLoc())](
                              IRBuilder<> &NB, Value *anti, Value *count) {
                            AllocaInst *replacement =
                                NB.CreateAlloca(NB.getInt8Ty(), count);
                    ...
                    auto rule2 = [funcName, TLI = this->TLI](
                                     IRBuilder<> &NB, Value *orig, Value *anti,
                                     ArrayRef<Value *>(args)) {
...
Value *GradientUtils::invertPointerM(Value *const oval, IRBuilder<> &BuilderM,
  ...
  if (isa<ConstantPointerNull>(oval)) {
    ...
    auto rule = [ty = CD->getType()](IRBuilder<> &BuilderM,
    ...
    auto rule = [](IRBuilder<> &BuilderM, ArrayRef<Constant *> Vals) {
      return ConstantVector::get(Vals);
    };
  ...
  if (isa<Argument>(oval) && cast<Argument>(oval)->hasByValAttr()) {
    ...
    auto rule1 = [oval, M](IRBuilder<> &bb) {
    ...
    if (!hasMetadata(arg, "enzyme_shadow")) {
      ...
      if ((mode == DerivativeMode::ReverseModeCombined ||
        ...
        endCheck:;
          if (!seen) {
            ...
            auto rule1 = [&arg, &allocaTy](IRBuilder<> &bb) {
              AllocaInst *antialloca = bb.CreateAlloca(
            ...
            auto rule2 = [&arg, &M, &oval, &len_arg, &val_arg,
                          &ty](IRBuilder<> &bb, Value *antialloca) {
      ...
      if (arg->hasInternalLinkage() || arg->hasPrivateLinkage() ||
        ...
        auto rule = [elemTy, arg](IRBuilder<> &BuilderM) {
    ...
    auto rule = [&arg, &shadowTy](IRBuilder<> &bb, Value *invertOp) {
    ...
    if (arg->isCast()) {
      ...
      if (isa<Constant>(ip)) {
        auto rule = [&arg](IRBuilder<> &bb, Value *ip) {
      ...
      if (auto C = dyn_cast<Constant>(ip)) {
        ...
        auto rule = [&arg, &invertargs](IRBuilder<> &bb, Value *ip) {
// TODO mark this the same inbounds as the original
    ...
    auto rule = [&arg](IRBuilder<> &bb, Value *ip0, Value *ip1) {
    ...
    auto rule = [&arg](IRBuilder<> &bb, Value *ip, Value *index) {
      return bb.CreateExtractElement(ip, index, arg->getName() + "'ipee");
      ;
    };

    Value *shadow =
        applyChainRule(arg->getType(), bb, rule, Gradient(ip), Primal(index));
    ...
    auto new_op2 = getNewFromOriginal(op2);

    auto rule = [Name = arg->getName()](IRBuilder<> &bb, Value *ip0, Value *ip1,
                                        Value *op2) {
      return bb.CreateInsertElement(ip0, ip1, op2, Name + "'ipie");
    };

    Value *shadow = applyChainRule(arg->getType(), bb, rule, Gradient(ip0),
                                   Gradient(ip1), Primal(new_op2));
    ...
    auto cond = getNewFromOriginal(arg->getCondition());

    auto rule = [cond, Name = arg->getName()](IRBuilder<> &bb, Value *tv,
                                              Value *fv) {
      return bb.CreateSelect(cond, tv, fv, Name + "'ipse");
    };
    ...
    auto rule = [&arg](IRBuilder<> &bb, Value *val0, Value *val1) {
    ...
    auto rule2 = [asize, M, inst](IRBuilder<> &bb, Value *antialloca) {
      auto dst_arg = bb.CreateBitCast(antialloca, bb.getInt8PtrTy());
      auto val_arg = bb.getInt8(0);
      auto len_arg =
          bb.CreateMul(bb.CreateZExtOrTrunc(asize, bb.getInt64Ty()),
                       bb.getInt64(M->getDataLayout().getTypeAllocSizeInBits(
                                       inst->getAllocatedType()) /
                                   8),
                       "", true, true);
      auto volatile_arg = bb.getFalse();
    ...
    case Intrinsic::nvvm_ldg_global_p:
    case Intrinsic::nvvm_ldg_global_f: {
      auto rule = [II, dbg = getNewFromOriginal(II->getDebugLoc())](
                      IRBuilder<> &bb, Value *ptr) {
        Value *args[] = {ptr};
    ...
    case Intrinsic::masked_load: {
      ...
      auto rule = [II, loc](IRBuilder<> &bb, Value *ptr, Value *align,
                            Value *mask, Value *defaultV) {
        llvm::SmallVector<unsigned int, 9> ToCopy2(MD_ToCopy);
        ToCopy2.push_back(LLVMContext::MD_noalias);
      ...
      return applyChainRule(II->getType(), bb, rule, Gradient(ptr),
                            Primal(align), Primal(mask), Gradient(defaultV));
    }
    }
...
void SubTransferHelper(GradientUtils *gutils, DerivativeMode mode,
  ...
  if (secretty) {
    ...
    if (mode == DerivativeMode::ReverseModeGradient ||
      ...
      if (srcConstant) {
        ...
        if (mode != DerivativeMode::ForwardModeSplit) {
          ...
          auto lshadow_dst = shadowsLookedUp
                                 ? shadow_dst
                                 : gutils->lookupM(shadow_dst, Builder2);
          auto llength = gutils->lookupM(length, Builder2);
          auto M = MTI->getParent()->getParent()->getParent();

          auto rule = [dstalign, M, &Context = MTI->getContext()](
                          IRBuilder<> &Builder2, Value *shadow_dst,
                          Value *length) {
            Value *args[] = {
              shadow_dst,
              Builder2.getInt8(0),
            ...
            if (args[0]->getType()->isIntegerTy())
              args[0] =
                  Builder2.CreateIntToPtr(args[0], Builder2.getInt8PtrTy());
...
#if LLVM_VERSION_MAJOR >= 10
              cal->addParamAttr(
                  0, Attribute::getWithAlignment(Context, Align(dstalign)));
#else
              cal->addParamAttr(0,
                                Attribute::getWithAlignment(Context, dstalign));
#endif
            }
          };

          gutils->applyChainRule(Builder2, rule, Gradient(lshadow_dst),
                                 Primal(llength));
        }
      } else {
        auto dsto =
        ...
        auto rule = [offset, mode, secretty, dstalign, srcalign, intrinsic,
                     M](IRBuilder<> &Builder2, Value *dsto, Value *srco,
                        Value *length) {
          ...
          if (mode == DerivativeMode::ForwardModeSplit) {
            ...
            auto dmemcpy = ((intrinsic == Intrinsic::memcpy)
                ...
                *M, secretty, dstalign, srcalign, dstaddr, srcaddr);
            Builder2.CreateCall(dmemcpy, args);
          }
        };

        gutils->applyChainRule(Builder2, rule, Gradient(dsto), Gradient(srco),
                               Primal(length));
      }
    }
  } else {
    ...
    if ((allowForward && (mode == DerivativeMode::ReverseModePrimal ||
      ...
      auto rule = [offset, dstalign, srcalign, isVolatile, intrinsic, M,
                   &MTI](IRBuilder<> &BuilderZ, Value *dsto, Value *srco,
                         Value *length) {
        if (dsto->getType()->isIntegerTy())
          dsto = BuilderZ.CreateIntToPtr(dsto, BuilderZ.getInt8PtrTy());
        ...
        if (srco->getType()->isIntegerTy())
          srco = BuilderZ.CreateIntToPtr(srco, BuilderZ.getInt8PtrTy());
        ...
        auto memtransIntr = Intrinsic::getDeclaration(M, intrinsic, tys);
        ...
        if (srcalign != 0) {
#if LLVM_VERSION_MAJOR >= 10
          cal->addParamAttr(1, Attribute::getWithAlignment(MTI->getContext(),
                                                           Align(srcalign)));
#else
          cal->addParamAttr(
      ...
      gutils->applyChainRule(BuilderZ, rule, Gradient(dsto), Gradient(srco),
                             Primal(length));
    }
  }
}
