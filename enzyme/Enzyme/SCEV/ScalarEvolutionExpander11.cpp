...
extern cl::opt<unsigned> llvm::SCEVCheapExpansionBudget;
/*
(
...
bool fake::SCEVExpander::isHighCostExpansionHelper(
  ...
  if (auto *UDivExpr = dyn_cast<SCEVUDivExpr>(S)) {
    ...
    if (auto *SC = dyn_cast<SCEVConstant>(UDivExpr->getRHS())) {
      if (SC->getAPInt().isPowerOf2()) {
        BudgetRemaining -= TTI.getArithmeticInstrCost(Instruction::LShr,
                                                      S->getType(), CostKind);
    ...
    BudgetRemaining -=
        TTI.getArithmeticInstrCost(Instruction::UDiv, S->getType(), CostKind);
  ...
  if (const SCEVNAryExpr *NAry = dyn_cast<SCEVNAryExpr>(S)) {
    ...
    switch (S->getSCEVType()) {
    case scAddExpr:
      PairCost = TTI.getArithmeticInstrCost(Instruction::Add, OpType, CostKind);
      break;
    case scMulExpr:
      // TODO: this is a very pessimistic cost modelling for Mul,
      // because of Bin Pow algorithm actually used by the expander,
      // see fake::SCEVExpander::visitMulExpr(), ExpandOpBinPowN().
      PairCost = TTI.getArithmeticInstrCost(Instruction::Mul, OpType, CostKind);
      break;
    case scSMaxExpr:
    case scUMaxExpr:
    case scSMinExpr:
    case scUMinExpr:
      PairCost =
          TTI.getCmpSelInstrCost(Instruction::ICmp, OpType,
