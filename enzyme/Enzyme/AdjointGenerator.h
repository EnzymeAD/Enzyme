  ...
  void handleBlas(llvm::CallInst &call, Function *called, StringRef funcName,
    ...
    if (funcName == "cblas_dgemm" || funcName == "cblas_sgemm") {
...
#if LLVM_VERSION_MAJOR >= 10
          BuilderZ.CreateMemCpy(
              arg1, MaybeAlign(),
              gutils->getNewFromOriginal(call.getArgOperand(7)), MaybeAlign(),
              Asize);
#else
          BuilderZ.CreateMemCpy(
              arg1, 0, gutils->getNewFromOriginal(call.getArgOperand(7)), 0,
              Asize);
#endif
        }
        if (Bcache) {
          ...
          BuilderZ.CreateMemCpy(
              arg2, MaybeAlign(),
              gutils->getNewFromOriginal(call.getArgOperand(9)), MaybeAlign(),
              Bsize);
#else
          BuilderZ.CreateMemCpy(
              arg2, 0, gutils->getNewFromOriginal(call.getArgOperand(9)), 0,
              Bsize);
#endif
        }
        if (Acache && Bcache) {
      ...
      if (Mode == DerivativeMode::ReverseModeCombined ||
        ...
        } else if (cast<ConstantInt>(call.getArgOperand(0))->getValue() ==
                   101) {
          if (aactive) {
          ...
          if (cast<ConstantInt>(call.getArgOperand(2))->getValue() == 112 ||
            ...
            if (cast<ConstantInt>(call.getArgOperand(0))->getValue() == 102)
              ...
              saldb = lookup(gutils->getNewFromOriginal(call.getArgOperand(5)),
                             Builder2);
          } else if (cast<ConstantInt>(call.getArgOperand(2))->getValue() ==
                     111) {
        ...
        if (bactive) {
          if (cast<ConstantInt>(call.getArgOperand(1))->getValue() == 112 ||
              cast<ConstantInt>(call.getArgOperand(1))->getValue() == 113) {
            ...
            if (cast<ConstantInt>(call.getArgOperand(0))->getValue() == 102)
              ...
              sblda = lookup(gutils->getNewFromOriginal(call.getArgOperand(3)),
                             Builder2);
          } else if (cast<ConstantInt>(call.getArgOperand(1))->getValue() ==
                     111) {
