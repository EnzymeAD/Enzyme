...
public:
  ...
  void visitMemSetInst(llvm::MemSetInst &MS) {
    ...
    if (Mode == DerivativeMode::ReverseModePrimal ||
      ...
      auto Defs = gutils->getInvertedBundles(&MS,
            {ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Primal}, BuilderZ, /*lookup*/false);
  ...
  void DifferentiableMemCopyFloats(CallInst &call, Value *origArg, Value *dsto,
                                   ...
                                   IRBuilder<> &Builder2, ArrayRef<OperandBundleDef> ReverseDefs) {
    size_t size = 1;
    ...
    if ((funcName == "cblas_ddot" || funcName == "cblas_sdot") &&
      ...
      if ((Mode == DerivativeMode::ReverseModeCombined ||
        ...
        if (xcache) {
          ...
          BuilderZ.CreateCall(dmemcpy, args, gutils->getInvertedBundles(&call,
            {ValueType::None, ValueType::Shadow, ValueType::None, ValueType::None, ValueType::None}, BuilderZ, /*lookup*/false));
        }
        if (ycache) {
          auto dmemcpy =
              getOrInsertMemcpyStrided(*gutils->oldFunc->getParent(),
          ...
          BuilderZ.CreateCall(dmemcpy, args, gutils->getInvertedBundles(&call,
            {ValueType::None, ValueType::None, ValueType::None, ValueType::Shadow, ValueType::None}, BuilderZ, /*lookup*/false));
        }
        if (xcache && ycache) {
        ...
        if (!gutils->isConstantValue(call.getArgOperand(3))) {
          ...
          Value * args1[6] = {
              ...
              lookup(gutils->getNewFromOriginal(call.getArgOperand(4)),
                     Builder2)};
          firstdcall = Builder2.CreateCall(derivcall, args1, gutils->getInvertedBundles(&call,
            {ValueType::None, xcache ? ValueType::None : ValueType::Primal, ValueType::None, ValueType::Shadow, ValueType::None}, Builder2, /*lookup*/true));
        }
        if (!gutils->isConstantValue(call.getArgOperand(1))) {
          ...
          Value * args2[6] = {
              ...
              lookup(gutils->getNewFromOriginal(call.getArgOperand(2)),
                     Builder2)};
          seconddcall = Builder2.CreateCall(derivcall, args2, gutils->getInvertedBundles(&call,
            {ValueType::None, ValueType::Shadow, ValueType::None, ycache ? ValueType::None : ValueType::Primal, ValueType::None}, Builder2, /*lookup*/true));
        }
  ...
  void handleMPI(llvm::CallInst &call, Function *called, StringRef funcName) {
    ...
    if (funcName == "PMPI_Isend" || funcName == "MPI_Isend" ||
      ...
      if (!gutils->isConstantInstruction(&call)) {
        ...
        if (Mode == DerivativeMode::ReverseModeGradient ||
          ...
          // becomes operand 0 for iwait.
          auto ReqDefs = gutils->getInvertedBundles(&call,
            {ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::Shadow}, Builder2, /*lookup*/true);

          auto BufferDefs = gutils->getInvertedBundles(&call,
            {ValueType::Shadow, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None}, Builder2, /*lookup*/true);
    ...
    if (funcName == "MPI_Wait" || funcName == "PMPI_Wait") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        // Need to preserve the shadow Request (operand 6 in isend/irecv), which
        // becomes operand 0 for iwait. However, this doesn't end up preserving the underlying
        // buffers for the adjoint. To rememdy, force inline.
        auto cal = Builder2.CreateCall(dwait, args, gutils->getInvertedBundles(&call,
            {ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::Shadow}, Builder2, /*lookup*/true));
    ...
    if (funcName == "MPI_Waitall" || funcName == "PMPI_Waitall") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        // Need to preserve the shadow Request (operand 6 in isend/irecv), which
        // becomes operand 0 for iwait. However, this doesn't end up preserving the underlying
        // buffers for the adjoint. To remedy, force inline the function.
        auto cal = Builder2.CreateCall(dwait, args, gutils->getInvertedBundles(&call,
            {ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::Shadow}, Builder2, /*lookup*/true));
        cal->setCallingConv(dwait->getCallingConv());
        cal->setDebugLoc(gutils->getNewFromOriginal(call.getDebugLoc()));
    ...
    if (funcName == "MPI_Recv" || funcName == "PMPI_Recv") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        auto Defs = gutils->getInvertedBundles(&call,
            {ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::None}, Builder2, /*lookup*/true);
        auto fcall = Builder2.CreateCall(
        ...
        auto MemsetDefs = gutils->getInvertedBundles(&call,
            {ValueType::Shadow, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None, ValueType::None}, Builder2, /*lookup*/true);
        auto memset = cast<CallInst>(Builder2.CreateCall(
    ...
    if (funcName == "MPI_Bcast") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        auto BufferDefs = gutils->getInvertedBundles(&call,
            {ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::Primal}, Builder2, /*lookup*/true);
        ...
        // 2. reduce sum diff(buffer) into intermediate
        {
          ...
          Builder2.CreateCall(
              called->getParent()->getOrInsertFunction("MPI_Reduce", FT), args, BufferDefs);
        }
...

        {
          ...
          auto mem = cast<CallInst>(Builder2.CreateCall(memcpyF, nargs, BufferDefs));
    ...
    if (funcName == "MPI_Reduce" || funcName == "PMPI_Reduce") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        auto BufferDefs = gutils->getInvertedBundles(&call,
          {ValueType::Shadow, ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::Primal}, Builder2, /*lookup*/true);
        ...
        // 2. MPI_Bcast intermediate to all
        {
          ...
          FunctionType *FT = FunctionType::get(call.getType(), types, false);
          Builder2.CreateCall(
              called->getParent()->getOrInsertFunction("MPI_Bcast", FT), args, BufferDefs);
        }
    ...
    if (funcName == "MPI_Allreduce") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        // Need to preserve the shadow send/recv buffers.
        auto BufferDefs = gutils->getInvertedBundles(&call,
          {ValueType::Shadow, ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::Primal}, Builder2, /*lookup*/true);
    ...
    if (funcName == "MPI_Gather") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        // Need to preserve the shadow send/recv buffers.
        auto BufferDefs = gutils->getInvertedBundles(&call,
          {ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Primal, ValueType::Primal}, Builder2, /*lookup*/true);
    ...
    if (funcName == "MPI_Scatter") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        // 2. Gather diff(recvbuffer) to intermediate buffer
        {
          ...
          FunctionType *FT = FunctionType::get(call.getType(), types, false);
          Builder2.CreateCall(
              called->getParent()->getOrInsertFunction("MPI_Gather", FT), args, BufferDefs);
        }
    ...
    if (funcName == "MPI_Allgather") {
      if (Mode == DerivativeMode::ReverseModeGradient ||
        ...
        // Need to preserve the shadow send/recv buffers.
        auto BufferDefs = gutils->getInvertedBundles(&call,
          {ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Shadow, ValueType::Primal, ValueType::Primal, ValueType::Primal}, Builder2, /*lookup*/true);
  ...
  void visitCallInst(llvm::CallInst &call) {
    ...
    if (Mode == DerivativeMode::ForwardMode) {
      ...
      auto Defs = gutils->getInvertedBundles(orig, BundleTypes, Builder2, /*lookup*/true);
    ...
    if (modifyPrimal) {
      ...
      if (Mode == DerivativeMode::ReverseModeCombined ||
        ...
        augmentcall = BuilderZ.CreateCall(FT, newcalled, pre_args, gutils->getInvertedBundles(orig, BundleTypes, BuilderZ, /*lookup*/false));
    ...
    CallInst *diffes = Builder2.CreateCall(FT, newcalled, args, gutils->getInvertedBundles(orig, BundleTypes, Builder2, /*lookup*/true));
    diffes->setCallingConv(orig->getCallingConv());
    diffes->setDebugLoc(gutils->getNewFromOriginal(orig->getDebugLoc()));
#if LLVM_VERSION_MAJOR >= 9
    for (auto pair : gradByVal) {
