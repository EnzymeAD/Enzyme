...
static inline bool is_value_needed_in_reverse(
  ...
  for (auto use : inst->users()) {
...
#endif
    {
      ...
      // Don't need the shadow argument if it is a pointer to pointers, which is only written
      // since the shadow pointer store will have been completed in the forward pass.
    if (writeOnlyNoCapture && TR.query(const_cast<Value*>(inst))[{-1,-1}] == BaseType::Pointer &&
