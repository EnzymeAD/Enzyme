...
template <typename T, bool llvmFunc = false>
mlir::FunctionType
...
FunctionOpInterface CloneFunctionWithReturns(
  ...
  if (auto llFTy = dyn_cast<LLVM::LLVMFunctionType>(F.getFunctionType())) {
    FTy = getFunctionTypeForClone<LLVM::LLVMFunctionType, true>(
        llFTy, mode, width, additionalArg, returnPrimals, returnShadows,
        RetActivity, ArgActivity);
  } else {
  ...
  */
  ...
  if (isa<LLVM::LLVMFuncOp>(F) && resultTypes.empty()) {
