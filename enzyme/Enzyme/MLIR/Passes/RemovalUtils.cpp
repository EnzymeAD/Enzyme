...
static inline bool isMovable(Operation *op) {
  return op->getNumRegions() == 0  && op->getBlock()->getTerminator() != op && mlir::isPure(op);
}
...
static Graph filterGraph(const Graph &Orig, const SetVector<Value> &Roots, const SetVector<T> &Required) {
  ...
  // Compute the graph with inverted edges by a floodfill, stopping at the first `required`.
  // This is required in the case of a root -> required -> required edge. We do not want to contain the required->required
  // subgraph.
  if (false) {
...
// The insertion point of rewriter must be in the reverse block, after any fwdrevmap
...
void mlir::enzyme::minCutCache(Block *forward, Block *reverse,
  ...
  LLVM_DEBUG(findCommonAncestor({forward->getParentOp(), reverse->getParentOp()})->dump());
  ...
  for (auto N : G) {
    if (!isa<Operation*>(N.first)) continue;
    auto op = cast<Operation*>(N.first);
    if (op->getBlock() != reverse) continue;
  ...
  if (newCaches.size()) {
     ...
     // llvm::sort(newCaches.getArrayRef().begin(), newCaches.getArrayRef().end(), mlir::enzyme::valueCmp);
	...
	if (Required.count(opNext)) continue;

	if (opNext->getNumResults() != 1) continue;
	   ...
	   if (newCache.getParentBlock() != forward) {
	    ...
	    enzyme::PushOp pushOp = ({
	      ...
	      rewriter.setInsertionPointAfterValue(newCache);
    	      rewriter.create<enzyme::PushOp>(newCache.getLoc(),
                                             initOp.getResult(), newCache);
	    });
	
	    assert(rewriter.getInsertionBlock() == reverse);
	    assert(rewriter.getInsertionPoint()->getBlock() == reverse);
	    enzyme::PopOp popOp = rewriter.create<enzyme::PopOp>(newCache.getLoc(), newCache.getType(), initOp.getResult());
  ...
  for (auto &info : caches) {
	  ...
	  if (!fwdGraph.contains(Node(&op))) continue;
	...
	OpBuilder::InsertionGuard guard(rewriter);
	  ...
	  bool hasUse = false;
	  for (auto &&[res, newRes] : llvm::zip_equal(op.getResults(), newO->getResults())) {
	    ...
	    enzyme::InitOp initOp = ({
	      ...
	      rewriter.create<enzyme::InitOp>(
		  newRes.getLoc(),
		  enzyme::CacheType::get(newRes.getContext(), newRes.getType()));
	    });
	    ...
	    enzyme::PopOp popOp = ({
	      ...
	      rewriter.create<enzyme::PopOp>(newRes.getLoc(), newRes.getType(), initOp.getResult());
	    });
    ...
    for (auto &op : llvm::make_early_inc_range(*forward)) {
      if (!cacheGraph.contains(Node(&op))) continue;
      bool hasUse = false;
	...
	hasUse = true;
      ...
      for (auto v : op.getOperands()) {
        ...
        if (v.getParentBlock() == forward) continue;
	...
	enzyme::InitOp initOp = ({
	      ...
	      OpBuilder::InsertionGuard guard(rewriter);
    	      ...
    	      rewriter.create<enzyme::PushOp>(v.getLoc(),
                                             initOp.getResult(), v);
	    };
	    ...
	    enzyme::PopOp popOp = ({
	      ...
	      rewriter.create<enzyme::PopOp>(v.getLoc(), v.getType(), initOp.getResult());
	    });
