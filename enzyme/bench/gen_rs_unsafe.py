...
def create_all_f(out_path, n_funcs, n_lines):
    ...
    f_out.write('unsafe fn f(vec: &mut [f32]) {\n')
    for i in range(n_funcs):
        f_out.write('    f_{}(vec);\n'.format(i))
    f_out.write('}\n\n')
...
def create_f(out_path, block, n_lines, offset):
    ...
    f_out.write('unsafe fn f_{}(vec: &mut [f32]) {{\n'.format(block))
    for i in range(n_lines):
        str_ = '    *vec.get_unchecked_mut({}) = '.format(i+offset)
        for j in range(2):
            str_ += 'vec.get_unchecked({})'.format(np.random.randint(0, i+offset))
