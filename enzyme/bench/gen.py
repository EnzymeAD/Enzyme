...
def create_all_f(out_path, n_funcs, n_lines):
    ...
    f_out.write('void f(vector<float>& vec) {\n')
    for i in range(n_funcs):
        f_out.write('    f_{}(vec);\n'.format(i))
    f_out.write('    return;\n')
    f_out.write('}\n\n')
...
def create_f(out_path, block, n_lines, offset):
    ...
    f_out.write('void f_{}(std::vector<float>& vec) {{\n'.format(block))
...
def create_main(file_mono, n_funcs, n_lines):
    ...
    f_out.write('void __enzyme_autodiff(void*, vector<float>&vec, vector<float>&dvec);\n')
...
def main():
    ...
    file_mono = 'stress_{}_{}.cpp'.format(n_funcs, n_lines)
