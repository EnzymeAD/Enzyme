...
define void @__enzyme_dreduce_float_3(float addrspace(3)* %ptr, float %0) #23 {
...
23:                                               ; preds = %1
  ...
  store float %24, float addrspace(3)* %26, align 4
  br label %27
...
adder:
  %prev = load float, float addrspace(3)* %ptr
  %next = fadd fast float %prev, %55
  store float %next, float addrspace(3)* %ptr
  br label %end
