  ...
  for (const Record *pattern : patterns) {
    ...
    if (intrinsic != MLIRDerivatives) {
      ...
      os << "    if (gutils->profiled && "
         ...
         << "        Value *numOperandsValue = ConstantInt::get(Int32Ty, "
            "numOperands);\n"
         << "        auto operands = isa<CallInst>(" << origName << ") ?\n"
         << "          cast<CallInst>(" << origName << ").args() : " << origName
         << ".operands();\n"
         << "        ArrayType *operandArrayType = ArrayType::get(DoubleTy, "
            "numOperands);\n"
         ...
         << "            operandValue = BuilderZ.CreateFPExt(origOp, "
            "DoubleTy);\n"
         << "          } else if (origOp->getType()->isIntegerTy()) {\n"
         << "            operandValue = BuilderZ.CreateSIToFP(origOp, "
            "DoubleTy);\n"
         << "          } else {\n"
         ...
         << "          {ConstantInt::get(Int32Ty, 0), "
            "ConstantInt::get(Int32Ty, 0)});\n"
         << "        bool isGPUTarget = " << origName
         << ".getModule()->getTargetTriple().isNVPTX();\n"
         << "        if (isGPUTarget) {\n"
         ...
         << origName << ".getContext()),\n"
         << "              {PtrTy, SizeTy, DoubleTy, SizeTy, PtrTy}, false);\n"
         << "          FunctionCallee logFunc = " << origName
         << ".getModule()->getOrInsertFunction(\"enzymeLogValueGPU\", "
            "LogValueFT);\n"
         << "          std::string funcName = "
            "gutils->oldFunc->getName().str();\n"
         << "          Module *M = " << origName << ".getModule();\n"
         << "          GlobalVariable *gv = M->getNamedGlobal(\"fpprofiled_\" "
            "+ funcName);\n"
         << "          if (!gv) {\n"
         << "            auto *strConst = "
            "ConstantDataArray::getString(M->getContext(), funcName, true);\n"
         << "            gv = new GlobalVariable(*M, strConst->getType(), "
            "false,\n"
         << "                GlobalValue::InternalLinkage, strConst, "
            "\"fpprofiled_\" + funcName,\n"
         << "                nullptr, GlobalVariable::NotThreadLocal, 1);\n"
         << "          }\n"
         << "          Value *funcNamePtr = BuilderZ.CreateAddrSpaceCast(\n"
         << "            BuilderZ.CreateInBoundsGEP(gv->getValueType(), gv,\n"
         << "              {BuilderZ.getInt32(0), BuilderZ.getInt32(0)}),\n"
         << "            PointerType::getUnqual(" << origName
         << ".getContext()));\n"
         << "          Value *numOpsSizeT = "
            "BuilderZ.CreateZExt(numOperandsValue, SizeTy);\n"
         << "          CallInst *logCallInst = BuilderZ.CreateCall(\n"
         << "            logFunc, {funcNamePtr, ConstantInt::get(SizeTy, "
            "instIdx), origValue, numOpsSizeT, operandPtrValue});\n"
    ...
    if (intrinsic != MLIRDerivatives) {
      ...
      os << "#ifdef ENABLE_POSEIDON\n"
         ...
         << origName << "), Builder2);\n"
         << "            Value *primalDouble = "
            "Builder2.CreateFPExt(primalInst, DoubleTy);\n"
         ...
         << "              FunctionType *LogGradFT = "
            "FunctionType::get(Type::getVoidTy("
         << origName
         << ".getContext()), {PtrTy, SizeTy, DoubleTy, DoubleTy}, false);\n"
         << "              FunctionCallee logFunc = " << origName
         << ".getModule()->getOrInsertFunction(\"enzymeLogGradGPU\", "
            "LogGradFT);\n"
         << "              std::string funcName = "
            "gutils->oldFunc->getName().str();\n"
         << "              Module *M = " << origName << ".getModule();\n"
         << "              GlobalVariable *gv = "
            "M->getNamedGlobal(\"fpprofiled_\" + funcName);\n"
         << "              if (!gv) {\n"
         << "                auto *strConst = "
            "ConstantDataArray::getString(M->getContext(), funcName, true);\n"
         << "                gv = new GlobalVariable(*M, strConst->getType(), "
         ...
         << "                    nullptr, GlobalVariable::NotThreadLocal, 1);\n"
         << "              }\n"
         << "              Value *funcNamePtr = Builder2.CreateAddrSpaceCast(\n"
         << "                Builder2.CreateInBoundsGEP(gv->getValueType(), "
            "gv,\n"
         << "                  {Builder2.getInt32(0), Builder2.getInt32(0)}),\n"
         << "                PointerType::getUnqual(" << origName
         << ".getContext()));\n"
         << "              CallInst *logCallInst = "
            "Builder2.CreateCall(logFunc, "
