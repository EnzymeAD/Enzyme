  ...
  for (Record *pattern : patterns)
  {
            ...
            ->isSubClassOf("ForwardFromSummedReverseInternal"))
    {
      ...
      else
      {
        ...
        //   << "           res = get1ULP(Builder2, gutils->getNewFromOriginal(&" << origName << "))";
      }
      ...
      for (auto argOpEn : enumerate(*argOps))
      {
        ...
        else
        {
          ...
          // error(f(x, y)) = max(  ulp(f(x, y)) , abs(  x / f(x, y) * df/dx * error(x) ) + abs(  y / f(x, y) * df/dy * error(y) ) )
          os << " if (Mode == DerivativeMode::ForwardModeError) {\n";
          os << "   dif = Builder2.CreateFDiv(Builder2.CreateFMul(dif, gutils->getNewFromOriginal(" << origName << ".getOperand(" << argIdx << "))), gutils->getNewFromOriginal(&" << origName << "));\n";
          os << " }\n";
          os << curIndent << INDENT
            ...
            [&](ArrayRef<unsigned> idx, Init *ival)
        {
          if (DagInit *resultTree = dyn_cast<DagInit>(ival))
          {
            ...
            else
            {
              ...
              os << curIndent << INDENT << INDENT << "if (Mode == DerivativeMode::ForwardModeError) {\n";
              os << curIndent << INDENT << INDENT << INDENT << "itmp = Builder2.CreateIntrinsic(Intrinsic::abs, ArrayRef<Type*>(itmp->getType()), ArrayRef<Value*>(itmp));\n";
              os << curIndent << INDENT << INDENT << INDENT << "}\n";
              os << curIndent << INDENT << INDENT
        ...
        if (intrinsic != MLIRDerivatives)
        {
          ...
          os << curIndent << INDENT << "if (Mode == DerivativeMode::ForwardModeError) {\n";
          os << curIndent << INDENT << INDENT << "res = Builder2.CreateMaxNum(get1ULP(Builder2, gutils->getNewFromOriginal(&" << origName << ")), res);\n";
        }
        os << "        }\n";
      }
    }
