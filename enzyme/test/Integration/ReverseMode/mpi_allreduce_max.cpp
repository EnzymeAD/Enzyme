...
void allreducemax_adjoint(float* sendbuf, float* dsendbuf, float* recvbuf, float* drecvbuf, int count){
    ...
    mpi_allreduce_comploc_float(sendbuf, recvbuf, recvlocbuf.data(), count, MPI_MAXLOC, MPI_COMM_WORLD);
    {
        ...
        MPI_Allreduce(drecvbuf, dsendbuf_tmp.data(), count, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);

        for(int i = 0; i < count; ++i) {
            if(recvlocbuf[i] == proc_id) {
                dsendbuf[i] += dsendbuf_tmp[i];
            }
        }
        std::fill(drecvbuf, drecvbuf + count, 0.0f);
    }
};
...
int main( int argc, char *argv[] )
{
    ...
    auto [sendbuf_a, dsendbuf_a, recvbuf_a, drecvbuf_a, count_a] = getTestData(n_procs, proc_id);
    {
        ...
        __enzyme_autodiff((void*)allreducemax, sendbuf_a.data(), dsendbuf_a.data(), recvbuf_a.data(), drecvbuf_a.data(), count_a);
    ...
    auto [sendbuf_b, dsendbuf_b, recvbuf_b, drecvbuf_b, count_b] = getTestData(n_procs, proc_id);
    {
        ...
        allreducemax_adjoint(sendbuf_b.data(), dsendbuf_b.data(), recvbuf_b.data(), drecvbuf_b.data(), count_b);
