; RUN: if [ %llvmver -ge 12 ]; then %opt < %s %newLoadEnzyme -passes="enzyme,mem2reg,correlated-propagation,adce,instsimplify,early-cse-memssa,simplifycfg,correlated-propagation,adce,jump-threading,instsimplify,early-cse,simplifycfg"  -enzyme-preopt=false -S | FileCheck -check-prefixes LL13,CHECK %s; fi
...
; CHECK-NEXT:   store double %tmp35, double* %2, align 8, !invariant.group !0
