...
; RUN: if [ %llvmver -lt 15 ]; then %opt < %s %newLoadEnzyme -passes="enzyme,mem2reg,gvn,simplifycfg,loop-deletion,simplifycfg,instsimplify,correlated-propagation,early-cse-memssa,adce"  -enzyme-preopt=false -S | FileCheck %s  -check-prefixes LL14,CHECK; fi
