...
; CHECK-NEXT:   %[[i0:.+]] = fcmp olt <2 x double> %a, %b
; CHECK-NEXT:   %[[i1:.+]] = fadd <2 x double> zeroinitializer, %differeturn
; CHECK-NEXT:   %[[i2:.+]] = select {{(fast )?}}<2 x i1> %[[i0]], <2 x double> zeroinitializer, <2 x double> %[[i1]]
; CHECK-NEXT:   %[[i3:.+]] = select {{(fast )?}}<2 x i1> %[[i0]], <2 x double> %[[i1]], <2 x double> zeroinitializer
; CHECK-NEXT:   %[[i4:.+]] = insertvalue { <2 x double>, <2 x double> } undef, <2 x double> %[[i2]], 0
; CHECK-NEXT:   %[[i5:.+]] = insertvalue { <2 x double>, <2 x double> } %[[i4]], <2 x double> %[[i3]], 1
; CHECK-NEXT:   ret { <2 x double>, <2 x double> } %[[i5]]
