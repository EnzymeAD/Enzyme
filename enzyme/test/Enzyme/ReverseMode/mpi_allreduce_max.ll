...
entry:
  call void (i8*, ...) @__enzyme_autodiff(i8* noundef bitcast (void (float*, float*, i32)* @mpi_allreduce_max_test to i8*), float* %sendbuf, float* %dsendbuf, float* %recvbuf, float* %drecvbuf, i32 %count)
...
; CHECK: define internal void @diffempi_allreduce_max_test(float* noundef %0, float* %"'", float* noundef %1, float* %"'1", i32 noundef %2) #0 {
; CHECK-NEXT: invert:
