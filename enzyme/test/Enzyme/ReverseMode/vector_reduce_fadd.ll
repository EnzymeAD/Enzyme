...
; CHECK-NEXT: entry:
; CHECK-NEXT:   %[[s0:.+]] = fadd float 0.000000e+00, %differeturn
...
; CHECK-NEXT:   %[[s1:.+]] = fadd <4 x float> zeroinitializer, %[[i1]]
; CHECK-NEXT:   %[[i2:.+]] = insertvalue { float, <4 x float> } undef, float %[[s0]], 0
; CHECK-NEXT:   %[[i3:.+]] = insertvalue { float, <4 x float> } %[[i2]], <4 x float> %[[s1]], 1
