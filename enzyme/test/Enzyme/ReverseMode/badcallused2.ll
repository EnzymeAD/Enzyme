...
; RUN: %opt < %s %newLoadEnzyme -passes="enzyme,mem2reg,instsimplify,adce,correlated-propagation,simplifycfg"  -enzyme-preopt=false -S | FileCheck %s
...
; CHECK-NEXT: entry:
; CHECK-NEXT:   %[[augsubf:.+]] = call i1 @augmented_subf(double* %x, double* %"x'")
; CHECK-NEXT:   %sel = select i1 %[[augsubf]], double 2.000000e+00, double 3.000000e+00
