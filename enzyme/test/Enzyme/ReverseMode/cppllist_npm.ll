...
; CHECK: define dso_local double @_Z10derivativedm(double %x, i64 %n)
; CHECK-NEXT:entry:
...
; CHECK-NEXT:  %1 = phi %class.node* [ %"'ipc.i", %for.body.i ], [ null, %entry ]
; CHECK-NEXT:  %list.09.i = phi %class.node* [ %2, %for.body.i ], [ null, %entry ]
; CHECK-NEXT:  %iv.next.i = add nuw nsw i64 %iv.i, 1
; CHECK-NEXT:  %call.i = call noalias nonnull dereferenceable(16) dereferenceable_or_null(16) i8* @_Znwm(i64 16) #14
; CHECK-NEXT:  %"call'mi.i" = call noalias nonnull dereferenceable(16) dereferenceable_or_null(16) i8* @_Znwm(i64 16) #14
; CHECK-NEXT:  call void @llvm.memset.p0i8.i64(i8* nonnull align 1 dereferenceable(16) dereferenceable_or_null(16) %"call'mi.i", i8 0, i64 16, i1 false) #5
; CHECK-NEXT:  %"'ipc.i" = bitcast i8* %"call'mi.i" to %class.node*
; CHECK-NEXT:  %2 = bitcast i8* %call.i to %class.node*
...
; CHECK-NEXT:  %5 = bitcast i8* %next.i.i to %class.node**
; CHECK-NEXT:  store %class.node* %1, %class.node** %"'ipc1.i", align 8
; CHECK-NEXT:  store %class.node* %list.09.i, %class.node** %5, align 8, !tbaa !8
; CHECK-NEXT:  %exitcond.i = icmp eq i64 %iv.i, %n
; CHECK-NEXT:  br i1 %exitcond.i, label %delete.end.i, label %for.body.i

; CHECK: delete.end.i:                                     ; preds = %for.body.i
; CHECK-NEXT:  call void @diffe_Z8sum_listPK4node(%class.node* nonnull %2, %class.node* nonnull %"'ipc.i", double 1.000000e+00) #5
; CHECK-NEXT:  br label %invertfor.body.i

; CHECK: invertfor.body.i:                                 ; preds = %delete.end.i, %incinvertfor.body.i
