...
; CHECK-NEXT:   %"sub13'de.0" = phi double [ %25, %invertif.end ], [ %19, %invertfor.body8 ]
; CHECK-NEXT:   %"'de.0" = phi double [ %26, %invertif.end ], [ %21, %invertfor.body8 ]
...
; CHECK-NEXT:   %"sub13'de.1" = phi double [ %25, %invertif.end.loopexit ], [ %19, %incinvertfor.body8 ]
; CHECK-NEXT:   %"'de.1" = phi double [ %26, %invertif.end.loopexit ], [ %21, %incinvertfor.body8 ]
; CHECK-NEXT:   %"iv1'ac.0" = phi i64 [ %_unwrap5, %invertif.end.loopexit ], [ %22, %incinvertfor.body8 ]
; CHECK-NEXT:   %9 = fneg fast double %"sub13'de.1"
...
; CHECK-NEXT:   %14 = load double, double* %"arrayidx11'ipg_unwrap", align 8
; CHECK-NEXT:   %15 = fadd fast double %14, %13
; CHECK-NEXT:   store double %15, double* %"arrayidx11'ipg_unwrap", align 8
; CHECK-NEXT:   %"arrayidx10'ipg_unwrap" = getelementptr inbounds double, double* %"L'", i64 %add_unwrap
; CHECK-NEXT:   %16 = load double, double* %"arrayidx10'ipg_unwrap", align 8
; CHECK-NEXT:   %17 = fadd fast double %16, %12
; CHECK-NEXT:   store double %17, double* %"arrayidx10'ipg_unwrap", align 8
; CHECK-NEXT:   %18 = icmp eq i64 %"iv1'ac.0", 0
; CHECK-NEXT:   %19 = select fast i1 %18, double 0.000000e+00, double %10
; CHECK-NEXT:   %20 = fadd fast double %"'de.1", %10
; CHECK-NEXT:   %21 = select fast i1 %18, double %20, double %"'de.1"
; CHECK-NEXT:   br i1 %18, label %invertfor.body, label %incinvertfor.body8
...
; CHECK-NEXT:   %_unwrap5 = add i64 %"iv'ac.0", -2
; CHECK-NEXT:   br label %invertfor.body8
...
; CHECK-NEXT:   %23 = load double, double* %"arrayidx14'ipg_unwrap", align 8
; CHECK-NEXT:   store double 0.000000e+00, double* %"arrayidx14'ipg_unwrap", align 8
; CHECK-NEXT:   %cmp2_unwrap = icmp ugt i64 %"iv'ac.0", 1
; CHECK-NEXT:   %24 = fadd fast double %"sub13'de.2", %23
; CHECK-NEXT:   %25 = select fast i1 %cmp2_unwrap, double %24, double %"sub13'de.2"
; CHECK-NEXT:   %26 = select fast i1 %cmp2_unwrap, double 0.000000e+00, double %23
