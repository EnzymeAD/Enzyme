; RUN: %opt < %s %newLoadEnzyme -passes="enzyme,inline,mem2reg,sroa,early-cse,adce,instsimplify,adce,simplifycfg,instsimplify,dse"  -enzyme-preopt=false -S | FileCheck %s
...
; CHECK-NEXT:   %1 = bitcast double** %array.i to i8*
; CHECK-NEXT:   call void @llvm.lifetime.start.p0i8(i64 8, i8* %1)
; CHECK-NEXT:   store double* null, double** %"array'ipa.i", align 8
; CHECK-NEXT:   %call_augmented.i = call { i8*, i8* } @augmented_allocateAndSet(double** %array.i, double** %"array'ipa.i", double %x, i32 %n) #5
; CHECK-NEXT:   %"'ipl.i" = load double*, double** %"array'ipa.i", align 8, !tbaa !2
; CHECK-NEXT:   %2 = load double*, double** %array.i, align 8, !tbaa !2
; CHECK-NEXT:   call void @diffeget(double* %2, double* %"'ipl.i", i32 3, double 1.000000e+00) #5
; CHECK-NEXT:   %3 = call { double } @diffeallocateAndSet(double** %array.i, double** %"array'ipa.i", double %x, i32 %n, double 1.000000e+00, { i8*, i8* } %call_augmented.i) #5
; CHECK-NEXT:   %4 = extractvalue { double } %3, 0
; CHECK-NEXT:   call void @llvm.lifetime.end.p0i8(i64 8, i8* %0)
; CHECK-NEXT:   call void @llvm.lifetime.end.p0i8(i64 8, i8* %1)
