; RUN: if [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -enzyme-preopt=false -enzyme-detect-readthrow=0 -enzyme -mem2reg -simplifycfg -dce -instsimplify -S | FileCheck %s; fi
; RUN: %opt < %s %newLoadEnzyme -enzyme-preopt=false -enzyme-detect-readthrow=0 -passes="enzyme,function(mem2reg,%simplifycfg,dce,instsimplify)" -S | FileCheck %s
...
; CHECK-NEXT:   %[[i3:.+]] = load <2 x double>, <2 x double>* %"other'de"
; CHECK-NEXT:   %[[i4:.+]] = fadd fast <2 x double> %[[i3]], %differeturn
; CHECK-NEXT:   call void @llvm.masked.store.v2f64.p0v2f64(<2 x double> %[[i4]], <2 x double>* %"other'de", i32 16, <2 x i1> %2)
; CHECK-NEXT:   %[[a3:.+]] = load <2 x double>, <2 x double>* %"other'de"
; CHECK-NEXT:   %[[a4:.+]] = insertvalue { <2 x double> } undef, <2 x double> %[[a3]], 0
; CHECK-NEXT:   ret { <2 x double> } %[[a4]]
; CHECK-NEXT: }
