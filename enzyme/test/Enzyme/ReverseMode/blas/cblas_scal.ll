...
declare dso_local float @__enzyme_autodifff(...)

declare float @cblas_sscal(i32, float, float*, i32)

define float @active(i32 %len, float %a, float* noalias %n, float* %dn, i32 %incn) {
entry:
  %call = call float (...) @__enzyme_autodifff(float (i32, float, float*, i32)* @f, i32 %len, float %a, float* noalias %n, float* %dn, i32 %incn)
  ret float %call 
}

define void @inactiveFirst(i32 %len, float %a, float* noalias %n, float* %dn, i32 %incn) {
entry:
  call void (...) @__enzyme_autodiff(float (i32, float, float*, i32)* @f, i32 %len, metadata !"enzyme_const", float %a, float* noalias %n, float* %dn, i32 %incn)
  ret void
}

define float @inactiveSecond(i32 %len, float %a, float* noalias %n, i32 %incn) {
entry:
  %call = call float (...) @__enzyme_autodifff(float (i32, float, float*, i32)* @f, i32 %len, float %a, metadata !"enzyme_const", float* noalias %n, i32 %incn)
  ret float %call
}

define float @activeMod(i32 %len, float %a, float* noalias %n, float* %dn, i32 %incn) {
entry:
  %call = call float (...) @__enzyme_autodifff(float (i32, float, float*, i32)* @f, i32 %len, float %a, float* noalias %n, float* %dn, i32 %incn)
  ret float %call
}

define void @inactiveModFirst(i32 %len, float %a, float* noalias %n, float* %dn, i32 %incn) {
entry:
  call void (...) @__enzyme_autodiff(float (i32, float, float*, i32)* @f, i32 %len, metadata !"enzyme_const", float %a, float* noalias %n, float* %dn, i32 %incn)
  ret void
}

define float @inactiveModSecond(i32 %len, float %a, float* noalias %n, i32 %incn) {
entry:
  %call = call float (...) @__enzyme_autodifff(float (i32, float, float*, i32)* @f, i32 %len, float %a, metadata !"enzyme_const", float* noalias %n, i32 %incn)
  ret float %call 
}

define float @f(i32 %len, float %a, float* noalias %n, i32 %incn) {
entry:
  %call = call float @cblas_sscal(i32 %len, float %a, float* %n, i32 %incn)
  ret float %call
}

define float @modf(i32 %len, float %a, float* noalias %n, i32 %incn) {
entry:
  %call = call float @f(i32 %len, float %a, float* %n, i32 %incn)
