...
; CHECK-NEXT:   store double 0.000000e+00, double* %15, align 8
...
; CHECK-NEXT:   %22 = bitcast i8* %lda_p to i64*
; CHECK-NEXT:   %23 = load i64, i64* %22, align 4
; CHECK-NEXT:   %24 = bitcast i8* %A to double*
...
; CHECK-NEXT:   %25 = icmp eq i64 %mul.i, 0
; CHECK-NEXT:   br i1 %25, label %__enzyme_memcpy_double_mat_64.exit, label %init.idx.i
...
; CHECK-NEXT:   %26 = mul nuw nsw i64 %j.i, %19
; CHECK-NEXT:   %27 = add nuw nsw i64 %i.i, %26
; CHECK-NEXT:   %dst.i.i = getelementptr inbounds double, double* %cache.A, i64 %27
; CHECK-NEXT:   %28 = mul nuw nsw i64 %j.i, %23
; CHECK-NEXT:   %29 = add nuw nsw i64 %i.i, %28
; CHECK-NEXT:   %dst.i1.i = getelementptr inbounds double, double* %24, i64 %29
...
; CHECK-NEXT:   %30 = icmp eq i64 %i.next.i, %19
; CHECK-NEXT:   br i1 %30, label %init.end.i, label %for.body.i

; CHECK: init.end.i:                                       ; preds = %for.body.i
; CHECK-NEXT:   %j.next.i = add nuw nsw i64 %j.i, 1
; CHECK-NEXT:   %31 = icmp eq i64 %j.next.i, %20
...
; CHECK-NEXT:   %32 = load double*, double** %0, align 8
...
; CHECK-NEXT:   %byref.constant.int.02 = alloca i64, align 8
; CHECK-NEXT:   %ldc = alloca i64, i64 1, align 16
; CHECK-NEXT:   %1 = bitcast i64* %ldc to i8*
; CHECK-NEXT:   %beta = alloca double, i64 1, align 16
; CHECK-NEXT:   %2 = bitcast double* %beta to i8*
...
; CHECK-NEXT:   store i8 %25, i8* %byref.transpose.transa, align 1
; CHECK-NEXT:   %ld.transb = load i8, i8* %malloccall1, align 1
...
; CHECK-NEXT:   store i8 %33, i8* %byref.transpose.transb, align 1
...
; CHECK-NEXT:   call void @dgemm_64_(i8* %byref.transpose.transa, i8* %malloccall1, i8* %k_p, i8* %n_p, i8* %m_p, i8* %alpha_p, i8* %17, i8* %lda_p, i8* %"C'", i8* %ldc_p, i8* %beta_p, i8* %"B'", i8* %ldb_p)
; CHECK-NEXT:   store i8 71, i8* %byref.constant.char.G, align 1
; CHECK-NEXT:   store i64 0, i64* %byref.constant.int.0, align 4
; CHECK-NEXT:   store i64 0, i64* %byref.constant.int.01, align 4
