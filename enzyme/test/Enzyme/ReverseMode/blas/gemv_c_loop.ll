...
declare double @__enzyme_autodiff(i8* noundef, ...)

define void @active(double* noundef %K, double* noundef %dK, double* nocapture readnone %m, double* noundef %x0, double* noundef %dx0, double* noundef %v0, double %T, i32 noundef %N) #0 {
entry:
  %call14 = tail call double (i8*, ...) @__enzyme_autodiff(i8* noundef nonnull bitcast (double (double*, double*, double*, double*, double, i32)* @coupled_springs to i8*), metadata !"enzyme_dup", double* noundef %K, double* noundef %dK, metadata !"enzyme_const", double* noundef %m, metadata !"enzyme_dup", double* noundef %x0, double* noundef %dx0, metadata !"enzyme_const", double* noundef %v0, metadata !"enzyme_const", double noundef 1.000000e+00, i32 noundef 15) #4
  ret void
}


; CHECK: define internal void @diffecoupled_springs(double* noundef %K, double* %"K'", double* nocapture readnone %m, double* noundef %x0, double* %"x0'", double* noundef %v0, double %T, i32 noundef %N, double 
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   %malloccall = tail call noalias nonnull i8* @malloc(i32 %mallocsize), !enzyme_cache_alloc ![[alloc1:[0-9]+]]
...
; CHECK-NEXT:   %malloccall2 = tail call noalias nonnull i8* @malloc(i32 %mallocsize1), !enzyme_cache_alloc ![[alloc2:[0-9]+]]
; CHECK-NEXT:   %[[i2:.+]] = load i8**, i8*** %malloccall2_cache, align 8, !dereferenceable ![[deref1:[0-9]+]], !invariant.group !2
; CHECK-NEXT:   %[[i3:.+]] = getelementptr inbounds i8*, i8** %[[i2]], i64 %iv
; CHECK-NEXT:   store i8* %malloccall2, i8** %[[i3]], align 8, !invariant.group ![[inv_grp1:[0-9]+]]
; CHECK-NEXT:   %[[i4:.+]] = load i8**, i8*** %malloccall_cache, align 8, !dereferenceable ![[deref1]], !invariant.group !5
; CHECK-NEXT:   %[[i5:.+]] = getelementptr inbounds i8*, i8** %[[i4]], i64 %iv
; CHECK-NEXT:   store i8* %malloccall, i8** %[[i5]], align 8, !invariant.group ![[inv_grp2:[0-9]+]]
...
; CHECK-NEXT:   %forfree = load i8**, i8*** %malloccall2_cache, align 8, !dereferenceable ![[deref1]], !invariant.group !2
...
; CHECK-NEXT:   %forfree17 = load i8**, i8*** %malloccall_cache, align 8, !dereferenceable ![[deref1]], !invariant.group !5
...
; CHECK-NEXT:   store double 0.000000e+00, double* %"'de", align 8
; CHECK-NEXT:   %[[i13:.+]] = load double, double* %"x0'", align 8, !alias.scope ![[scope:[0-9]+]], !noalias ![[noalias:[0-9]+]]
; CHECK-NEXT:   %[[i14:.+]] = fadd fast double %[[i13]], %[[i12]]
; CHECK-NEXT:   store double %[[i14]], double* %"x0'", align 8, !alias.scope ![[scope]], !noalias ![[noalias]]
...
; CHECK-NEXT:   %[[i16:.+]] = load i8**, i8*** %malloccall2_cache, align 8, !dereferenceable ![[deref1]], !invariant.group !2
; CHECK-NEXT:   %[[i17:.+]] = getelementptr inbounds i8*, i8** %[[i16]], i64 %[[i15]]
; CHECK-NEXT:   %[[i18:.+]] = load i8*, i8** %[[i17]], align 8, !invariant.group ![[inv_grp1]]
; CHECK-NEXT:   %cache.x_unwrap = bitcast i8* %[[i18]] to double*
; CHECK-NEXT:   %[[i19:.+]] = load i8**, i8*** %malloccall_cache, align 8, !dereferenceable ![[deref1]], !invariant.group !5
; CHECK-NEXT:   %[[i20:.+]] = getelementptr inbounds i8*, i8** %[[i19]], i64 %[[i15]]
; CHECK-NEXT:   %[[i21:.+]] = load i8*, i8** %[[i20]], align 8, !invariant.group ![[inv_grp2]]
; CHECK-NEXT:   %cache.A_unwrap = bitcast i8* %[[i21]] to double*
; CHECK-NEXT:   %[[i22:.+]] = bitcast double* %cache.A_unwrap to i8*
; CHECK-NEXT:   tail call void @free(i8* nonnull %[[i22]])
; CHECK-NEXT:   %[[i23:.+]] = bitcast double* %cache.x_unwrap to i8*
; CHECK-NEXT:   tail call void @free(i8* nonnull %[[i23]])
; CHECK-NEXT:   %[[i24:.+]] = load i64, i64* %"iv'ac", align 4
; CHECK-NEXT:   %[[i25:.+]] = icmp eq i64 %[[i24]], 0
; CHECK-NEXT:   %[[i26:.+]] = xor i1 %[[i25]], true
; CHECK-NEXT:   br i1 %[[i25]], label %invertentry, label %incinvertfor.body

; CHECK: incinvertfor.body:                                ; preds = %invertfor.body
; CHECK-NEXT:   %[[i27:.+]] = load i64, i64* %"iv'ac", align 4
; CHECK-NEXT:   %[[i28:.+]] = add nsw i64 %[[i27]], -1
; CHECK-NEXT:   store i64 %[[i28]], i64* %"iv'ac", align 4
; CHECK-NEXT:   br label %invertfor.body
; CHECK-NEXT: }
