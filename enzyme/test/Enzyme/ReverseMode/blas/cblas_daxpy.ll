...
define void @active(i32 %n, double %alpha, double* %x, double* %_x, i32 %incx, double* %y, double* %_y, i32 %incy) {
entry:
  %call = tail call double @__enzyme_autodiff(i8* bitcast (void (i32, double, double*, i32, double*, i32)* @wrapper to i8*), i32 %n, double %alpha, double* %x, double* %_x, i32 %incx, double* %y, double* %_y, i32 %incy)
  ret void
}

declare double @__enzyme_autodiff(i8*, i32, double, double*, double*, i32, double*, double*, i32)

define void @inactive_alpha(i32 %n, double %alpha, double* %x, double* %_x, i32 %incx, double* %y, double* %_y, i32 %incy) {
entry:
  %0 = load i32, i32* @enzyme_const, align 4
  tail call void @__enzyme_autodiff1(i8* bitcast (void (i32, double, double*, i32, double*, i32)* @wrapper to i8*), i32 %n, i32 %0, double %alpha, double* %x, double* %_x, i32 %incx, double* %y, double* %_y, i32 %incy)
  ret void
}
...
;CHECK:define internal void @[[inactive_alpha]](i32 %n, double %alpha, double* %x, double* %"x'", i32 %incx, double* %y, double* %"y'", i32 %incy)
;CHECK-NEXT:entry:
;CHECK-NEXT:  tail call void @cblas_daxpy(i32 %n, double %alpha, double* %x, i32 %incx, double* %y, i32 %incy)
;CHECK-NEXT:  call void @cblas_daxpy(i32 %n, double %alpha, double* %"y'", i32 %incy, double* %"x'", i32 %incx)
;CHECK-NEXT:  ret void
