...
entry:
  %call = tail call float @cblas_sasum(i32 %n, float* %x, i32 %incx)
  ret float %call
}

declare float @cblas_sasum(i32, float*, i32)
...
;CHECK-NEXT:entry:
;CHECK-NEXT:  %call = tail call float @cblas_sasum(i32 %n, float* %x, i32 %incx)
;CHECK-NEXT:  call void @__enzyme_memcpy_floatasum_adjoint(i32 %n, float %differeturn, float* %x, float* %"x'", i32 %incx)
;CHECK-NEXT:  ret void
;CHECK-NEXT:}
...
;CHECK-NEXT:  store float 0.000000e+00, float* %"x'", align 4
;CHECK-NEXT:  call void @__enzyme_memcpy_floatasum_adjoint(i32 %n, float %differeturn, float* %1, float* %"x'", i32 1)
;CHECK-NEXT:  ret void
;CHECK-NEXT:}
