...
; CHECK-DAG:   %m = call double @__enzyme_fprt_64_52_binop_fmul(double %y, double %y, i64 8, i64 23, i64 1, {{.*}}i8{{.*}})
...
; CHECK-DAG:   %y = load double, double* %x, align 8
; CHECK-DAG:   %m = call double @__enzyme_fprt_64_52_binop_fmul(double %y, double %y, i64 8, i64 23, i64 2, {{.*}}i8{{.*}})
; CHECK-DAG:   store double %m, double* %x, align 8
; CHECK-DAG:   ret void
; CHECK-DAG: }
...
; CHECK-DAG:   %y = load double, double* %x, align 8
; CHECK-DAG:   %m = call double @__enzyme_fprt_64_52_binop_fmul(double %y, double %y, i64 3, i64 7, i64 2, {{.*}}i8{{.*}})
; CHECK-DAG:   store double %m, double* %x, align 8
; CHECK-DAG:   ret void
; CHECK-DAG: }
