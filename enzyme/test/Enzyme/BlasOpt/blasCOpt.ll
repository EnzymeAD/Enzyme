;RUN: if [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -blasOpt -S | FileCheck %s; fi
;RUN: %opt < %s %newLoadEnzyme -passes="blasOpt" -S | FileCheck %s
...
; CHECK-COM: Target function:
; CHECK: define void @f(i8* noalias %C, i8* noalias %alpha, i8* noalias %beta, i64 %m, i64 %n, i64 %p, i64 %lda, i64 %ldb, i64 %ldc, i64 %incx, i64 %incy, i64 %incv, i64 %incw)
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   %1 = fmul double %0, %alpha
