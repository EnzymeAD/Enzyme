...
; CHECK-NEXT:   %[[i3:.+]] = fdiv double %[[i2]], %y
; CHECK-NEXT:   %[[i4:.+]] = call double @llvm.fabs.f64(double %[[i3]])
; CHECK-NEXT:   %[[i5:.+]] = fmul double %"y'", %y
; CHECK-NEXT:   %[[i6:.+]] = fdiv double %[[i5]], %[[i0]]
; CHECK-NEXT:   %[[i7:.+]] = fdiv double %[[i6]], %y
; CHECK-NEXT:   %[[i8:.+]] = fdiv double %x, %y
; CHECK-NEXT:   %[[i9:.+]] = fmul double %[[i7]], %[[i8]]
; CHECK-NEXT:   %[[i10:.+]] = fneg double %[[i9]]
; CHECK-NEXT:   %[[i11:.+]] = call double @llvm.fabs.f64(double %[[i10]])
; CHECK-NEXT:   %[[i12:.+]] = fadd double %[[i4]], %[[i11]]
; CHECK-NEXT:   %[[i13:.+]] = bitcast double %[[i0]] to i64
; CHECK-NEXT:   %[[i14:.+]] = xor i64 %[[i13]], 1
; CHECK-NEXT:   %[[i15:.+]] = bitcast i64 %[[i14]] to double
; CHECK-NEXT:   %[[i16:.+]] = fsub double %[[i0]], %[[i15]]
; CHECK-NEXT:   %[[i17:.+]] = call double @llvm.fabs.f64(double %[[i16]])
; CHECK-NEXT:   %[[i18:.+]] = call double @llvm.maxnum.f64(double %[[i17]], double %[[i12]])
