; RUN: if [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -simplify-gvn -S | FileCheck %s; fi
; RUN: %opt < %s %newLoadEnzyme -passes="simplify-gvn" -S | FileCheck %s

; Test that optimization is NOT applied when argument has non-load/store uses

declare void @external_func(i32*)

define i32 @test_call_use(i32* noalias nocapture %ptr) {
entry:
  store i32 42, i32* %ptr, align 4
  call void @external_func(i32* %ptr)
  %val = load i32, i32* %ptr, align 4
  ret i32 %val
}

; CHECK: define i32 @test_call_use(i32* noalias nocapture %ptr)
; CHECK-NEXT: entry:
; CHECK-NEXT:   store i32 42, i32* %ptr, align 4
; CHECK-NEXT:   call void @external_func(i32* %ptr)
; CHECK-NEXT:   %val = load i32, i32* %ptr, align 4
; CHECK-NEXT:   ret i32 %val
