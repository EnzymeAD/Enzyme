...
; RUN: if [ %llvmver -ge 16 ]; then %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,instcombine,correlated-propagation,adce,instcombine,simplifycfg,early-cse,simplifycfg,loop-unroll,instcombine,simplifycfg,gvn,jump-threading,instcombine,simplifycfg)" -enzyme-preopt=false -S | FileCheck %s; fi
