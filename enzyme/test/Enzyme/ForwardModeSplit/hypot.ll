; RUN: if [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -mem2reg -sroa -instcombine -early-cse -simplifycfg -adce -S | FileCheck %s; fi
; RUN: %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,sroa,instcombine,early-cse,%simplifycfg,adce)" -enzyme-preopt=false -S | FileCheck %s
...
; CHECK-DAG:   %[[a2:.+]] = fmul double %"y'", %y
; CHECK-DAG:   %[[a40:.+]] = fdiv double %[[a1]], %[[a0]]
; CHECK-DAG:   %[[a41:.+]] = fdiv double %[[a2]], %[[a0]]
; CHECK-DAG:   %[[a3:.+]] = fadd double %[[a40]], %[[a41]]
