...
; RUN: if [ %llvmver -ge 16 ]; then %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,instsimplify,instcombine,simplifycfg)" -enzyme-preopt=false -S | FileCheck %s; fi
