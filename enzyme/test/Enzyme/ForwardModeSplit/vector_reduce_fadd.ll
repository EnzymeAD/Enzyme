...
entry:
  %0 = tail call float (float (float, <4 x float>)*, ...) @__enzyme_fwdsplit(float (float, <4 x float>)* nonnull @tester, float %start_value, float 1.0, <4 x float> %input, <4 x float> <float 1.0, float 1.0, float 1.0, float 1.0>)
  ret float %0
}
