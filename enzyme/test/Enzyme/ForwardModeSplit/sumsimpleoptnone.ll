; RUN: if [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -inline -mem2reg -instsimplify -adce -loop-deletion -correlated-propagation -simplifycfg -S -early-cse | FileCheck %s; fi
; RUN: %opt < %s %newLoadEnzyme -passes="enzyme,inline,function(mem2reg,instsimplify,adce,loop-deletion,correlated-propagation,simplifycfg, -early-cse)" -enzyme-preopt=false -S | FileCheck %s
