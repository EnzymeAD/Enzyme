...
entry:
  tail call void (...) @__enzyme_fwddiff.f64(void (double*, double*, i64)* nonnull @memcpy_float, metadata !"enzyme_width", i64 3, metadata !"enzyme_const", double* %dst, double* %src, double* %srcp1, double* %dsrcp2, double* %dsrcp3, i64 %n) #3
  ret void
}
...
; CHECK: define internal void @fwddiffe3memcpy_float(double* nocapture %dst, double* nocapture readonly %src, [3 x double*] %"src'", i64 %num)
; CHECK-NEXT: entry:
