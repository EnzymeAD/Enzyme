...
; CHECK-NEXT:   %[[b0:.+]] = fmul double %[[a0]], %x
; CHECK-NEXT:   %[[a1:.+]] = extractvalue [3 x double] %"x'", 1
; CHECK-NEXT:   %[[b1:.+]] = fmul double %[[a1]], %x
; CHECK-NEXT:   %[[a2:.+]] = extractvalue [3 x double] %"x'", 2
; CHECK-NEXT:   %[[b2:.+]] = fmul double %[[a2]], %x
; CHECK-NEXT:   %[[a3:.+]] = extractvalue [3 x double] %"y'", 0
; CHECK-NEXT:   %[[b3:.+]] = fmul double %[[a3]], %y
; CHECK-NEXT:   %[[a4:.+]] = extractvalue [3 x double] %"y'", 1
; CHECK-NEXT:   %[[b4:.+]] = fmul double %[[a4]], %y
; CHECK-NEXT:   %[[a5:.+]] = extractvalue [3 x double] %"y'", 2
; CHECK-NEXT:   %[[b5:.+]] = fmul double %[[a5]], %y

; CHECK-NEXT:   %[[c0:.+]] = fadd double %[[b0]], %[[b3]]
; CHECK-NEXT:   %[[c1:.+]] = fadd double %[[b1]], %[[b4]]
; CHECK-NEXT:   %[[c2:.+]] = fadd double %[[b2]], %[[b5]]

; CHECK-NEXT:   %[[a6:.+]] = call double @cabs([2 x double] %agg1)

; CHECK-NEXT:   %[[r0:.+]] = fdiv double %[[c0]], %[[a6]]
; CHECK-NEXT:   %[[a12:.+]] = insertvalue [3 x double] undef, double %[[r0]], 0
; CHECK-NEXT:   %[[r1:.+]] = fdiv double %[[c1]], %[[a6]]
; CHECK-NEXT:   %[[a18:.+]] = insertvalue [3 x double] %[[a12]], double %[[r1]], 1
; CHECK-NEXT:   %[[r2:.+]] = fdiv double %[[c2]], %[[a6]]
