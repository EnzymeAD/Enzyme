...
%struct.Gradients = type { [2 x double], [2 x double], [2 x double] }
...
define %struct.Gradients @test_derivative([2 x double] %x) {
entry:
  %0 = tail call %struct.Gradients ([2 x double] ([2 x double])*, ...) @__enzyme_fwddiff([2 x double] ([2 x double])*  @tester,  metadata !"enzyme_width", i64 3, [2 x double] %x, [2 x double] [ double 1.0, double 1.0 ], [2 x double] [ double 1.0, double 1.0 ], [2 x double] [ double 1.0, double 1.0 ])
  ret %struct.Gradients %0
}


; CHECK: define internal [3 x [2 x double]] @fwddiffe3tester([2 x double] %in, [3 x [2 x double]] %"in'")
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   %[[a19:.+]] = extractvalue [3 x [2 x double]] %"in'", 0, 0
; CHECK-NEXT:   %[[a31:.+]] = extractvalue [3 x [2 x double]] %"in'", 1, 0
; CHECK-NEXT:   %[[a43:.+]] = extractvalue [3 x [2 x double]] %"in'", 2, 0
; CHECK-NEXT:   %[[a20:.+]] = extractvalue [3 x [2 x double]] %"in'", 0, 1
; CHECK-NEXT:   %[[a32:.+]] = extractvalue [3 x [2 x double]] %"in'", 1, 1
; CHECK-NEXT:   %[[a44:.+]] = extractvalue [3 x [2 x double]] %"in'", 2, 1
...
; CHECK-NEXT:   %[[r00:.+]] = insertvalue [3 x [2 x double]] undef, double %[[a23]], 0, 0
; CHECK-NEXT:   %[[r01:.+]] = insertvalue [3 x [2 x double]] %[[r00]], double %[[a27]], 0, 1

; CHECK-NEXT:   %[[r10:.+]] = insertvalue [3 x [2 x double]] %[[r01]], double %[[a35]], 1, 0
; CHECK-NEXT:   %[[r11:.+]] = insertvalue [3 x [2 x double]] %[[r10]], double %[[a39]], 1, 1

; CHECK-NEXT:   %[[r20:.+]] = insertvalue [3 x [2 x double]] %[[r11]], double %[[a47]], 2, 0
; CHECK-NEXT:   %[[r21:.+]] = insertvalue [3 x [2 x double]] %[[r20]], double %[[a51]], 2, 1
