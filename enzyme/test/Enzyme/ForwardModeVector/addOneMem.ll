...
; RUN: if [ %llvmver -lt 16 ]; %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -inline -mem2reg -instsimplify -gvn -dse -dse -S | FileCheck %s; fi
; RUN: if [ %llvmver -ge 16 ]; %opt < %s %newLoadEnzyme -passes="enzyme,function(inline,mem2reg,instsimplify,gvn,dse,dse)" -enzyme-preopt=false -S | FileCheck %s; fi
