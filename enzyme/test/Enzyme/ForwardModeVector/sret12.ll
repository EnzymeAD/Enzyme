...
; RUN: if [ %llvmver -ge 12 ] && [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -enzyme-detect-readthrow=0 -mem2reg -simplifycfg -early-cse -adce -S | FileCheck %s ; fi
; RUN: if [ %llvmver -ge 12 ]; then %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,%simplifycfg,early-cse,adce)" -enzyme-preopt=false -enzyme-detect-readthrow=0 -S | FileCheck %s; fi
...
; CHECK: define {{[^@]+}}@fwddiffe3_Z6squared(%"struct.std::array"* noalias nocapture align 8 "enzyme_sret"="{{[0-9]+}}" [[AGG_RESULT:%.*]], [3 x %"struct.std::array"*] "enzyme_sret_v"="{{[0-9]+}}" %"agg.result'", double [[X:%.*]], [3 x double] %"x'") 
; CHECK-NEXT:  entry:
