;RUN: if [ %llvmver -lt 16 ]; %opt < %s %loadEnzyme -enzyme -mem2reg -instsimplify -simplifycfg -S | FileCheck %s; fi
;RUN: if [ %llvmver -ge 16 ]; %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,instsimplify,simplifycfg)" -S | FileCheck %s; fi
