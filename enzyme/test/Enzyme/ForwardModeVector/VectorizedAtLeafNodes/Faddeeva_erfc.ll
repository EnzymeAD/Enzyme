...
entry:
  %0 = tail call { <3 x double>, <3 x double> } ({ double, double } ({ double, double })*, ...) @__enzyme_fwddiff({ double, double } ({ double, double })* @tester,  metadata !"enzyme_width", i64 3, { double, double } %x, { <3 x double>, <3 x double> } { <3 x double> <double 1.0, double 1.0, double 1.0>, <3 x double> <double 0.0, double 1.0, double 1.0> })
...
; CHECK-NEXT:   %.splatinsert1 = insertelement <3 x double> {{(poison|undef)}}, double %15, i32 0
...
; CHECK-NEXT:   %.splatinsert3 = insertelement <3 x double> {{(poison|undef)}}, double %14, i32 0
; CHECK-NEXT:   %.splat4 = shufflevector <3 x double> %.splatinsert3, <3 x double> {{(poison|undef)}}, <3 x i32> zeroinitializer
; CHECK-NEXT:   %.splatinsert5 = insertelement <3 x double> {{(poison|undef)}}, double %15, i32 0
