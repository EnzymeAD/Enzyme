...
; RUN: %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -enzyme-vectorize-at-leaf-nodes -inline -mem2reg -instsimplify -adce -loop-deletion -correlated-propagation -simplifycfg -S -early-cse | FileCheck %s
...
; CHECK: define internal void @fwddiffe3f(double* %x, <3 x double>* %"x'", double** %y, <3 x double>** %"y'", i64 %n) #1 {
; CHECK-NEXT: entry:
