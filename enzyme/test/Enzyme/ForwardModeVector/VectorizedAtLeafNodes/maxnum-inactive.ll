...
entry:
  %0 = tail call <2 x double> (double (double, double)*, ...) @__enzyme_fwddiff(double (double, double)* nonnull @tester, metadata !"enzyme_width", i64 2, double %x, <2 x double> <double 1.0, double 0.0>, metadata !"enzyme_const", double %y)
  ret <2 x double> %0
}
...
; CHECK-NEXT:   %1 = select fast i1 %0, <2 x double> %"x'", <2 x double> zeroinitializer
