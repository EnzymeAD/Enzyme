...
define dso_local <3 x double> @dsumsquare(double %x) local_unnamed_addr #3 {
entry:
  %0 = tail call <3 x double> (double (double)*, ...) @__enzyme_fwddiff(double (double)* nonnull @foo, metadata !"enzyme_width", i64 3, double %x, <3 x double> <double 1.0, double 2.0, double 3.0>)
  ret <3 x double> %0
}
...
; CHECK: define internal <3 x double> @fwddiffe3foo(double %x, <3 x double> %"x'")
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = call fast <3 x double> @fwddiffe3sub(double %x, <3 x double> %"x'")
