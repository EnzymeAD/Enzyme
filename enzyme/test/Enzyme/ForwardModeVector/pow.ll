...
; CHECK-NEXT:   %7 = fcmp fast oeq double %x, 0.000000e+00
; CHECK-NEXT:   %8 = fcmp fast olt double 0.000000e+00, %y
; CHECK-NEXT:   %9 = and i1 %7, %8
; CHECK-NEXT:   %10 = call fast double @llvm.pow.f64(double %x, double %y)
; CHECK-NEXT:   %11 = call fast double @llvm.log.f64(double %x)
; CHECK-NEXT:   %12 = fmul fast double %10, %11
; CHECK-NEXT:   %13 = select fast i1 %9, double 0.000000e+00, double %12
; CHECK-NEXT:   %14 = extractvalue [2 x double] %"y'", 0
; CHECK-NEXT:   %15 = fmul fast double %14, %13
; CHECK-NEXT:   %[[i14:.+]] = extractvalue [2 x double] %"y'", 1
; CHECK-NEXT:   %[[i15:.+]] = fmul fast double %[[i14]], %13
; CHECK-NEXT:   %[[i12:.+]] = fadd fast double %4, %15
