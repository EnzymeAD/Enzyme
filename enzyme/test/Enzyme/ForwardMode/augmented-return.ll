...
; CHECK-NEXT:   %3 = extractvalue { i8*, double } %2, 1
; CHECK-NEXT:   %4 = fmul fast double %3, %o
; CHECK-NEXT:   %5 = fmul fast double %3, %o
...
; CHECK-NEXT: entry:
; CHECK-NEXT:   %m = tail call noalias nonnull dereferenceable(64) dereferenceable_or_null(64) i8* @malloc(i64 64) #0
; CHECK-NEXT:   %0 = tail call noalias nonnull dereferenceable(64) dereferenceable_or_null(64) i8* @malloc(i64 64) #0
; CHECK-NEXT:   %a = fadd double %x, %x
; CHECK-NEXT:   %".fca.0.insert'ipiv" = insertvalue { i8*, double } undef, i8* %0, 0
; CHECK-NEXT:   %.fca.0.insert = insertvalue { i8*, double } undef, i8* %m, 0
; CHECK-NEXT:   %".fca.1.insert'ipiv" = insertvalue { i8*, double } %".fca.0.insert'ipiv", double %a, 1
; CHECK-NEXT:   %.fca.1.insert = insertvalue { i8*, double } %.fca.0.insert, double %a, 1
; CHECK-NEXT:   %1 = insertvalue { { i8*, double }, { i8*, double } } undef, { i8*, double } %.fca.1.insert, 0
; CHECK-NEXT:   %2 = insertvalue { { i8*, double }, { i8*, double } } %1, { i8*, double } %".fca.1.insert'ipiv", 1
; CHECK-NEXT:   ret { { i8*, double }, { i8*, double } } %2
; CHECK-NEXT: }