...
; RUN: %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,instsimplify,simplify-cfg)" -enzyme-preopt=false -S | FileCheck %s -check-prefix=MSG1
; RUN: %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,instsimplify,simplify-cfg)" -enzyme-preopt=false -S | FileCheck %s -check-prefix=MSG2
