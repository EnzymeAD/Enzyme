...
; CHECK-NEXT:   %malloccall4 = tail call noalias nonnull i8* @malloc(i64 %12)
; CHECK-NEXT:   %13 = call i32 @MPI_Comm_rank(%struct.ompi_communicator_t* bitcast (%struct.ompi_predefined_communicator_t* @ompi_mpi_comm_world to %struct.ompi_communicator_t*), i32* %4)
; CHECK-NEXT:   %14 = load i32, i32* %4, align 4
; CHECK-NEXT:   %15 = icmp sgt i32 %2, 0
; CHECK-NEXT:   br i1 %15, label %_loop, label %_endloop

; CHECK: _loop:                                            ; preds = %_endthen, %3
; CHECK-NEXT:   %16 = phi i32 [ 0, %3 ], [ %26, %_endthen ]
; CHECK-NEXT:   %17 = getelementptr inbounds i32, i32* %7, i32 %16
; CHECK-NEXT:   %18 = load i32, i32* %17, align 4
; CHECK-NEXT:   %19 = icmp eq i32 %18, %14
; CHECK-NEXT:   br i1 %19, label %_then, label %_endthen

; CHECK: _then:                                            ; preds = %_loop
; CHECK-NEXT:   %20 = bitcast i8* %"'ipc" to float*
; CHECK-NEXT:   %21 = getelementptr inbounds float, float* %20, i32 %16
; CHECK-NEXT:   %22 = load float, float* %21, align 4
; CHECK-NEXT:   br label %_endthen

; CHECK: _endthen:                                         ; preds = %_then, %_loop
; CHECK-NEXT:   %23 = phi fast float [ 0.000000e+00, %_loop ], [ %22, %_then ]
; CHECK-NEXT:   %24 = bitcast i8* %malloccall4 to float*
; CHECK-NEXT:   %25 = getelementptr inbounds float, float* %24, i32 %16
; CHECK-NEXT:   store float %23, float* %25, align 4
; CHECK-NEXT:   %26 = add i32 %16, 1
; CHECK-NEXT:   %27 = icmp slt i32 %16, %2
; CHECK-NEXT:   br i1 %27, label %_loop, label %_endloop

; CHECK: _endloop:                                         ; preds = %_endthen, %3
; CHECK-NEXT:   %28 = call i32 @MPI_Allreduce(i8* %malloccall4, i8* %"'ipc3", i32 %2, %struct.ompi_datatype_t* bitcast (%struct.ompi_predefined_datatype_t* @ompi_mpi_float to %struct.ompi_datatype_t*), %struct.ompi_op_t* bitcast (%struct.ompi_predefined_op_t* @ompi_mpi_op_sum to %struct.ompi_op_t*), %struct.ompi_communicator_t* bitcast (%struct.ompi_predefined_communicator_t* @ompi_mpi_comm_world to %struct.ompi_communicator_t*))
; CHECK-NEXT:   tail call void @free(i8* nonnull %malloccall4)
; CHECK-NEXT:   %29 = bitcast i32* %7 to i8*
; CHECK-NEXT:   tail call void @free(i8* nonnull %29)
; CHECK-NEXT:   ret void
; CHECK-NEXT: }
