...
; CHECK-NEXT:   %"sum.019'" = phi {{(fast )?}}double [ %[[i3:.+]], %for.cond.cleanup4 ], [ 0.000000e+00, %entry ]
...
; CHECK-NEXT:   %"sum.0.lcssa'" = phi {{(fast )?}}double [ 0.000000e+00, %entry ], [ %[[i3]], %for.cond.cleanup4 ]
...
; CHECK-NEXT:   %"sum.116'" = phi {{(fast )?}}double [ %[[i3]], %for.body5 ], [ %"sum.019'", %for.cond1.preheader ]
...
; CHECK-NEXT:   %[[i3]] = fadd fast double %[[i2]], %"sum.116'"
