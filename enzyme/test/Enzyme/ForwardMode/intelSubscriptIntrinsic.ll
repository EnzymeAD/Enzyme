; RUN: if [ %llvmver -lt 16 ] && [ %llvmver -ge 13 ]; then %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -S | FileCheck %s; fi
; RUN: if [ %llvmver -ge 13 ]; then %opt < %s %newLoadEnzyme -passes="enzyme" -enzyme-preopt=false -S | FileCheck %s; fi
...
entry:
  tail call void @__enzyme_fwddiff(float (...)* bitcast (float (%"QNCA_a0$float*$rank1$"*)* @selectfirst to float (...)*), %"QNCA_a0$float*$rank1$"* nonnull %X, %"QNCA_a0$float*$rank1$"* nonnull %DX)
  ret void
}

declare void @__enzyme_fwddiff(float (...)* noalias, %"QNCA_a0$float*$rank1$"* noalias readonly dereferenceable(72) "ptrnoalias", %"QNCA_a0$float*$rank1$"* noalias dereferenceable(72) "ptrnoalias") local_unnamed_addr
