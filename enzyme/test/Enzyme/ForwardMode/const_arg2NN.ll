...
entry:
  %0 = tail call double (...) @__enzyme_fwddiff(double (double, i8*)* @julia_g_6797_inner.1, metadata !"enzyme_dup", double %x, double 1.0, metadata !"enzyme_dupnoneed", i8* null, i8* null)
  ret double %0
}
...
; CHECK-NEXT:   %0 = call fast double @fwddiffejulia_f_kw__kw_6800(double* undef, double* %"a2'ipc", double %a0, double %"a0'")
