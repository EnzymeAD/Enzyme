...
declare void @__enzyme_observe(double, double (double, double, double)*, i8*, double, double)
...
entry:
  ...
  call void @__enzyme_observe(double 20.0, double (double, double, double)* @normal_logpdf, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64 0), double %mu, double 1.0)
  ret void
}
...
; CHECK-NEXT:   %likelihood. = call double @normal_logpdf(double %0, double 1.000000e+00, double 2.000000e+01)
; CHECK-NEXT:   %log_prob_sum1 = load double, double* %likelihood
; CHECK-NEXT:   %4 = fadd double %log_prob_sum1, %likelihood.
; CHECK-NEXT:   store double %4, double* %likelihood
; CHECK-NEXT:   call void @__enzyme_insert_choice(i8* %trace, i8* nocapture readonly getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64 0), double %likelihood., i8* inttoptr (i64 4626322717216342016 to i8*), i64 8)
; CHECK-NEXT:   ret void
; CHECK-NEXT: }