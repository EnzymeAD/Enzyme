...
; CHECK-NEXT:   %likelihood.mu = call double @normal_logpdf(double 0.000000e+00, double 1.000000e+00, double %sample.mu)
; CHECK-NEXT:   %0 = alloca double
; CHECK-NEXT:   store double %sample.mu, double* %0
; CHECK-NEXT:   %1 = bitcast double* %0 to i8**
; CHECK-NEXT:   %2 = load i8*, i8** %1
; CHECK-NEXT:   call void @__enzyme_insert_choice(i8* %trace, i8* getelementptr inbounds ([3 x i8], [3 x i8]* @.str, i64 0, i64 0), double %likelihood.mu, i8* %2, i64 8)
; CHECK-NEXT:   %sample.x = call double @normal(double %sample.mu, double 1.000000e+00)
; CHECK-NEXT:   %likelihood.x = call double @normal_logpdf(double %sample.mu, double 1.000000e+00, double %sample.x)
; CHECK-NEXT:   %3 = alloca double
; CHECK-NEXT:   store double %sample.x, double* %3
; CHECK-NEXT:   %4 = bitcast double* %3 to i8**
; CHECK-NEXT:   %5 = load i8*, i8** %4
; CHECK-NEXT:   call void @__enzyme_insert_choice(i8* %trace, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64 0), double %likelihood.x, i8* %5, i64 8)
