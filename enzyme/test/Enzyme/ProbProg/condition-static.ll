...
; CHECK: define internal { double, i8* } @condition_loss(double* %data, i32 %n, i8* %0)
; CHECK-NEXT: entry:
; CHECK-NEXT:   %trace = call i8* @__enzyme_newtrace()
; CHECK-NEXT:   %1 = call i1 @__enzyme_has_choice(i8* %0, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64 0))
; CHECK-NEXT:   br i1 %1, label %condition.call.with.trace, label %condition.call.without.trace

; CHECK: condition.call.with.trace:                        ; preds = %entry
; CHECK-NEXT:   %2 = alloca double, align 8
; CHECK-NEXT:   %3 = bitcast double* %2 to i8*
; CHECK-NEXT:   %4 = call i64 @__enzyme_get_choice(i8* %0, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64 0), i8* %3, i64 8)
...
; CHECK-NEXT:   %6 = phi double [ %5, %condition.call.with.trace ], [ %sample.call, %condition.call.without.trace ]
; CHECK-NEXT:   %likelihood.call = call double @normal_logpdf(double 0.000000e+00, double 1.000000e+00, double %6)
; CHECK-NEXT:   %7 = alloca double, align 8
; CHECK-NEXT:   store double %6, double* %7, align 8
; CHECK-NEXT:   %8 = bitcast double* %7 to i8**
; CHECK-NEXT:   %9 = load i8*, i8** %8, align 8
; CHECK-NEXT:   call void @__enzyme_insert_choice(i8* %trace, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64 0), double %likelihood.call, i8* %9, i64 8)
; CHECK-NEXT:   %10 = call i1 @__enzyme_has_choice(i8* %0, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.2, i64 0, i64 0))
; CHECK-NEXT:   br i1 %10, label %condition.call1.with.trace, label %condition.call1.without.trace

; CHECK: condition.call1.with.trace:                       ; preds = %entry.cntd
; CHECK-NEXT:   %11 = alloca double, align 8
; CHECK-NEXT:   %12 = bitcast double* %11 to i8*
; CHECK-NEXT:   %13 = call i64 @__enzyme_get_choice(i8* %0, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.2, i64 0, i64 0), i8* %12, i64 8)
; CHECK-NEXT:   %14 = load double, double* %11, align 8
...
; CHECK-NEXT:   %15 = phi double [ %14, %condition.call1.with.trace ], [ %sample.call1, %condition.call1.without.trace ]
; CHECK-NEXT:   %likelihood.call1 = call double @normal_logpdf(double 0.000000e+00, double 1.000000e+00, double %15)
; CHECK-NEXT:   %16 = alloca double, align 8
; CHECK-NEXT:   store double %15, double* %16, align 8
; CHECK-NEXT:   %17 = bitcast double* %16 to i8**
; CHECK-NEXT:   %18 = load i8*, i8** %17, align 8
; CHECK-NEXT:   call void @__enzyme_insert_choice(i8* %trace, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.2, i64 0, i64 0), double %likelihood.call1, i8* %18, i64 8)
; CHECK-NEXT:   %19 = call i1 @__enzyme_has_call(i8* %0, i8* getelementptr inbounds ([21 x i8], [21 x i8]* @0, i32 0, i32 0))
; CHECK-NEXT:   br i1 %19, label %condition.call2.with.trace, label %condition.call2.without.trace

; CHECK: condition.call2.with.trace:                       ; preds = %entry.cntd.cntd
; CHECK-NEXT:   %20 = call i8* @__enzyme_get_trace(i8* %0, i8* getelementptr inbounds ([21 x i8], [21 x i8]* @0, i32 0, i32 0))
; CHECK-NEXT:   %call22 = call { double, i8* } @condition_calculate_loss(double %6, double %15, double* %data, i32 %n, i8* %20)
; CHECK-NEXT:   br label %entry.cntd.cntd.cntd

; CHECK: condition.call2.without.trace:                    ; preds = %entry.cntd.cntd
; CHECK-NEXT:   %call23 = call { double, i8* } @condition_calculate_loss(double %6, double %15, double* %data, i32 %n, i8* null)
; CHECK-NEXT:   br label %entry.cntd.cntd.cntd

; CHECK: entry.cntd.cntd.cntd:                             ; preds = %condition.call2.without.trace, %condition.call2.with.trace
; CHECK-NEXT:   %21 = phi { double, i8* } [ %call22, %condition.call2.with.trace ], [ %call23, %condition.call2.without.trace ]
; CHECK-NEXT:   %22 = extractvalue { double, i8* } %21, 0
; CHECK-NEXT:   %23 = extractvalue { double, i8* } %21, 1
; CHECK-NEXT:   call void @__enzyme_insert_call(i8* %trace, i8* getelementptr inbounds ([21 x i8], [21 x i8]* @0, i32 0, i32 0), i8* %23)
; CHECK-NEXT:   %mrv = insertvalue { double, i8* } undef, double %22, 0
; CHECK-NEXT:   %mrv1 = insertvalue { double, i8* } %mrv, i8* %trace, 1
; CHECK-NEXT:   ret { double, i8* } %mrv1
; CHECK-NEXT: }


; CHECK: define internal { double, i8* } @condition_calculate_loss(double %m, double %b, double* %data, i32 %n, i8* %0)
; CHECK-NEXT: entry:
; CHECK-NEXT:   %trace = call i8* @__enzyme_newtrace()
...
; CHECK-NEXT:   %loss.0.lcssa = phi double [ 0.000000e+00, %entry ], [ %13, %for.body.cntd ]
...
; CHECK-NEXT:   %loss.021 = phi double [ 0.000000e+00, %for.body.preheader ], [ %13, %for.body.cntd ]
; CHECK-NEXT:   %1 = trunc i64 %indvars.iv to i32
; CHECK-NEXT:   %conv2 = sitofp i32 %1 to double
; CHECK-NEXT:   %mul1 = fmul double %conv2, %m
; CHECK-NEXT:   %2 = fadd double %mul1, %b
; CHECK-NEXT:   %3 = call i1 @__enzyme_has_choice(i8* %0, i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str, i64 0, i64 0))
; CHECK-NEXT:   br i1 %3, label %condition.call.with.trace, label %condition.call.without.trace

; CHECK: condition.call.with.trace:                        ; preds = %for.body
; CHECK-NEXT:   %4 = alloca double, align 8
; CHECK-NEXT:   %5 = bitcast double* %4 to i8*
; CHECK-NEXT:   %6 = call i64 @__enzyme_get_choice(i8* %0, i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str, i64 0, i64 0), i8* %5, i64 8)
; CHECK-NEXT:   %7 = load double, double* %4, align 8
; CHECK-NEXT:   br label %for.body.cntd

; CHECK: condition.call.without.trace:                     ; preds = %for.body
; CHECK-NEXT:   %sample.call = call double @normal(double %2, double 1.000000e+00)
; CHECK-NEXT:   br label %for.body.cntd

; CHECK: for.body.cntd:                                    ; preds = %condition.call.without.trace, %condition.call.with.trace
; CHECK-NEXT:   %8 = phi double [ %7, %condition.call.with.trace ], [ %sample.call, %condition.call.without.trace ]
; CHECK-NEXT:   %likelihood.call = call double @normal_logpdf(double %2, double 1.000000e+00, double %8)
; CHECK-NEXT:   %9 = alloca double, align 8
; CHECK-NEXT:   store double %8, double* %9, align 8
; CHECK-NEXT:   %10 = bitcast double* %9 to i8**
; CHECK-NEXT:   %11 = load i8*, i8** %10, align 8
; CHECK-NEXT:   call void @__enzyme_insert_choice(i8* %trace, i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str, i64 0, i64 0), double %likelihood.call, i8* %11, i64 8)
; CHECK-NEXT:   %arrayidx3 = getelementptr inbounds double, double* %data, i64 %indvars.iv
; CHECK-NEXT:   %12 = load double, double* %arrayidx3, align 8
; CHECK-NEXT:   %sub = fsub double %8, %12
; CHECK-NEXT:   %mul2 = fmul double %sub, %sub
; CHECK-NEXT:   %13 = fadd double %mul2, %loss.021
