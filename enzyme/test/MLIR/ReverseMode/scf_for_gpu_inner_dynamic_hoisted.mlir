...
// CHECK-NEXT:    %alloc = gpu.alloc(%[[varg1]], %[[varg2]]) : memref<?x?xf32, 1>
// CHECK-NEXT:    %[[v0:.+]] = scf.for %[[varg4:.+]] = %c0 to %[[varg1]] step %c1 iter_args(%[[varg5:.+]] = %cst) -> (f32) {
// CHECK-NEXT:      %subview = memref.subview %alloc[%[[varg4]], 0] [1, %[[varg2]]] [1, 1] : memref<?x?xf32, 1> to memref<?xf32, strided<[1], offset: ?>, 1>
// CHECK-NEXT:      %[[v2:.+]] = scf.for %[[varg6:.+]] = %c0 to %[[varg2]] step %c1 iter_args(%[[varg7:.+]] = %[[varg5]]) -> (f32) {
// CHECK-NEXT:        memref.store %[[varg7]], %subview[%[[varg6]]] : memref<?xf32, strided<[1], offset: ?>, 1>
...
// CHECK-NEXT:      %subview = memref.subview %alloc[%[[v3]], 0] [1, %[[varg2]]] [1, 1] : memref<?x?xf32, 1> to memref<?xf32, strided<[1], offset: ?>, 1>
...
// CHECK-NEXT:        %[[v7:.+]] = memref.load %subview[%[[v6]]] : memref<?xf32, strided<[1], offset: ?>, 1>
