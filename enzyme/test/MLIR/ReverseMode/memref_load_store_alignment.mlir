...
func.func @loadstore(%arg0: memref<?xf32>, %arg1: memref<?xf32>) {
  ...
  %v = memref.load %arg0[%c0] alignment(16) : memref<?xf32>
  %sq = arith.mulf %v, %v : f32
  memref.store %sq, %arg1[%c1] alignment(16) : memref<?xf32>
  return
}

// CHECK-LABEL: func.func @loadstore(
// The store adjoint loads and zeroes the store's shadow slot.
// CHECK:         memref.load %arg3[%c1] alignment(16) : memref<?xf32>
// CHECK:         memref.store %cst, %arg3[%c1] alignment(16) : memref<?xf32>
// The load adjoint accumulates into the load's shadow slot.
// CHECK:         memref.load %arg1[%c0] alignment(16) : memref<?xf32>
// CHECK:         memref.store %{{.+}}, %arg1[%c0] alignment(16) : memref<?xf32>
