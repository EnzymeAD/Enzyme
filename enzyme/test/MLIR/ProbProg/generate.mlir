...
// CHECK-NEXT:    %[[trace2:.+]] = enzyme.addSampleToTrace %[[sample_from_constraint]] into %[[trace1]] {symbol = #enzyme.symbol<2>} : (!enzyme.Trace, tensor<f64>) -> !enzyme.Trace
// CHECK-NEXT:    %[[trace3:.+]] = enzyme.addWeightToTrace %[[addf2]] into %[[trace2]] : (!enzyme.Trace, tensor<f64>) -> !enzyme.Trace
// CHECK-NEXT:    %[[final_trace:.+]] = enzyme.addRetvalToTrace %[[sample_from_constraint]] into %[[trace3]] : (!enzyme.Trace, tensor<f64>) -> !enzyme.Trace
