...
module {
  ...
  func.func @test(%rng : memref<i64>, %mean : f64, %stddev : f64) -> f64 {
    %s = enzyme.sample @normal(%rng, %mean, %stddev) { symbol = 1 : ui64, name="s" } : (memref<i64>, f64, f64) -> f64
    %t = enzyme.sample @normal(%rng, %s, %stddev) { symbol = 2 : ui64, name="t" } : (memref<i64>, f64, f64) -> f64
    return %t : f64
  }
