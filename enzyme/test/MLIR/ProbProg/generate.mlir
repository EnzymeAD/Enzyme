...
module {
  ...
  func.func @generate(%rng : tensor<2xui64>, %mean : tensor<f64>, %stddev : tensor<f64>) -> (tensor<f64>, tensor<2xui64>, tensor<f64>, tensor<f64>) {
    ...
    %res:6 = enzyme.generate @test(%rng, %mean, %stddev) given %0 { name = "test", traced_output_indices = array<i64: 0>, constrained_symbols = [#enzyme.symbol<1>, #enzyme.symbol<2>] } : (tensor<2xui64>, tensor<f64>, tensor<f64>) -> (!enzyme.Trace, tensor<f64>, tensor<f64>, tensor<2xui64>, tensor<f64>, tensor<f64>)
...
// CHECK-NEXT:     %10 = enzyme.addRetvalToTrace(%5#0 : tensor<f64>) into %9
