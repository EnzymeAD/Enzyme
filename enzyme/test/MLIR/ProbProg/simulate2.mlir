...
// CHECK-NEXT:    %[[trace2:.+]] = enzyme.addSubtrace %[[two_normals_call]]#0 into %[[trace1]] {symbol = #enzyme.symbol<2>} : (!enzyme.Trace, !enzyme.Trace) -> !enzyme.Trace
// CHECK-NEXT:    %[[addf2:.+]] = arith.addf %[[addf1]], %[[two_normals_call]]#1 : tensor<f64>
// CHECK-NEXT:    %[[trace3:.+]] = enzyme.addSampleToTrace %[[two_normals_call]]#3, %[[two_normals_call]]#4 into %[[trace2]] {symbol = #enzyme.symbol<2>} : (!enzyme.Trace, tensor<f64>, tensor<f64>) -> !enzyme.Trace
// CHECK-NEXT:    %[[trace4:.+]] = enzyme.addWeightToTrace %[[addf2]] into %[[trace3]] : (!enzyme.Trace, tensor<f64>) -> !enzyme.Trace
// CHECK-NEXT:    %[[final_trace:.+]] = enzyme.addRetvalToTrace %[[two_normals_call]]#3, %[[two_normals_call]]#4 into %[[trace4]] : (!enzyme.Trace, tensor<f64>, tensor<f64>) -> !enzyme.Trace
...
// CHECK-NEXT:    %[[trace1:.+]] = enzyme.addSampleToTrace %[[normal_call1]]#1 into %[[trace_init]] {symbol = #enzyme.symbol<3>} : (!enzyme.Trace, tensor<f64>) -> !enzyme.Trace
...
// CHECK-NEXT:    %[[trace2:.+]] = enzyme.addSampleToTrace %[[normal_call2]]#1 into %[[trace1]] {symbol = #enzyme.symbol<4>} : (!enzyme.Trace, tensor<f64>) -> !enzyme.Trace
// CHECK-NEXT:    %[[trace3:.+]] = enzyme.addWeightToTrace %[[addf2]] into %[[trace2]] : (!enzyme.Trace, tensor<f64>) -> !enzyme.Trace
// CHECK-NEXT:    %[[final_trace:.+]] = enzyme.addRetvalToTrace %[[normal_call1]]#1, %[[normal_call2]]#1 into %[[trace3]] : (!enzyme.Trace, tensor<f64>, tensor<f64>) -> !enzyme.Trace
