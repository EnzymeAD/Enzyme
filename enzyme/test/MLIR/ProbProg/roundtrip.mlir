...
module {
  ...
  func.func @generate(%mean : f64, %stddev : f64) -> f64 {
    ...
    %trace1, %res1 = enzyme.trace @test(%trace0, %res0, %stddev) { name = "test" } : (!enzyme.Trace<f64>, f64, f64) -> (!enzyme.Trace<f64>, f64)
    return %res1 : f64
  }
}
...
// CHECK-NEXT:    %[[trace1:.+]], %[[res1:.+]] = enzyme.trace @test(%[[trace0]], %[[res0]], %[[stddev]]) {name = "test"} : (!enzyme.Trace<f64>, f64, f64) -> (!enzyme.Trace<f64>, f64)
