// Reverse mode through FIR's by-reference memory model. This is the HLFIR for
//
...
// The adjoint of the output flows back through the shadow buffers, giving
// dx += 2*x*dr.
//
// REQUIRES: fir_enzyme_plugin
// RUN: %fir_enzyme %s --pass-pipeline='builtin.module(enzyme-wrap{infn=sq outfn= argTys=enzyme_dup,enzyme_dup retTys= mode=ReverseModeCombined},canonicalize,remove-unnecessary-enzyme-ops,canonicalize)' | FileCheck %s
...
// CHECK: %[[SEED:.*]] = arith.addf %[[G]], %{{.*}} fastmath<fast> : f32
// CHECK: fir.store %{{.*}} to %[[DRD]]#0 : !fir.ref<f32>
// Backprop through mul: contributions to each loaded x.
// CHECK: arith.mulf %[[SEED]], %[[X1]] fastmath<fast> : f32
// CHECK: arith.mulf %[[SEED]], %[[X0]] fastmath<fast> : f32
