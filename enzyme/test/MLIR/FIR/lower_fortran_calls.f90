...
! primal alone).
!
! REQUIRES: fir_enzyme_plugin, flang_enzyme_mlir
!
! fir-opt + FIREnzyme.
! RUN: %flang_fc1 -emit-hlfir %s -o - | %fir_enzyme --pass-pipeline='builtin.module(enzyme-lower-fortran-calls)' | FileCheck %s
!
! flang + FlangEnzymeMLIR: the plugin adds the same pass to flang's own
! HLFIR-to-FIR pipeline, followed by `enzyme`, so by -emit-fir the fwddiff op has
! already been replaced by a call to the generated derivative.
! RUN: %flang_enzyme -emit-fir %s -o - | FileCheck %s --check-prefix=FLANG
...
! FLANG-LABEL: func.func @_QPdriver
! FLANG: call @fwddiffe_QPsquare(
! FLANG-NOT: call @_QPf__enzyme_fwddiff
! FLANG-LABEL: func.func private @fwddiffe_QPsquare
