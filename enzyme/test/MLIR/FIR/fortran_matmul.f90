...
! intrinsic to a first-class hlfir.matmul op, and fir-opt with the FIREnzyme
! plugin loaded ingests that HLFIR and runs the `enzyme` differentiation pass
! over it in an FIR/HLFIR-aware context.
!
...
! RUN: %flang_fc1 -emit-hlfir %s -o - | %fir_enzyme --pass-pipeline='builtin.module(enzyme)' | FileCheck %s
