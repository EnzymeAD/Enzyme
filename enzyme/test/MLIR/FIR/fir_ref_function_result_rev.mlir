// Reverse mode through a Fortran function whose result flows through a local
// fir.alloca result buffer. This is the HLFIR for
//
//   real function square(x); real,intent(in)::x; square = x*x; end
//
// The return adjoint seeds the shadow result buffer, propagates back through the
// assign and the two loads of x, and accumulates 2*x*seed into the shadow x.
//
// REQUIRES: fir_enzyme_plugin
// RUN: %fir_enzyme %s --pass-pipeline='builtin.module(enzyme-wrap{infn=square outfn= argTys=enzyme_dup retTys=enzyme_active mode=ReverseModeCombined},canonicalize,remove-unnecessary-enzyme-ops,canonicalize)' | FileCheck %s
