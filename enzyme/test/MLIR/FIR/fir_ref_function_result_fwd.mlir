// Forward mode through a Fortran function whose result flows through a local
// fir.alloca result buffer. This is the HLFIR for
//
//   real function square(x); real,intent(in)::x; square = x*x; end
//
// For the tangent to reach the returned value, activity analysis has to treat
// the alloca as pointer-like memory (AutoDiffTypeInterface::isMutable) and give
// it a shadow buffer. d/dx (x*x) = 2*x, so the returned tangent is 2 * x * dx.
//
