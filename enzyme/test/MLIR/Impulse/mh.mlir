...
module {
  ...
  func.func @test(%rng : tensor<2xui64>, %mean : tensor<f64>, %stddev : tensor<f64>) -> (tensor<2xui64>, tensor<f64>) {
    ...
    %t:2 = impulse.sample @normal(%s#0, %s#1, %stddev) { logpdf = @logpdf, symbol = #impulse.symbol<2>, name="t" } : (tensor<2xui64>, tensor<f64>, tensor<f64>) -> (tensor<2xui64>, tensor<f64>)
  ...
  func.func @mh(%rng : tensor<2xui64>, %mean : tensor<f64>, %stddev : tensor<f64>) -> (tensor<1x2xf64>, tensor<f64>, tensor<2xui64>) {
    ...
    %res:3 = scf.for %i = %c0 to %c1000 step %c1 iter_args(%trace = %init_trace, %weight = %init_weight, %rng1 = %rng) -> (tensor<1x2xf64>, tensor<f64>, tensor<2xui64>) {
      %step1:4 = impulse.mh @test(%rng1, %mean, %stddev) given %trace weight %weight
          { name = "mh_1", selection = [[#impulse.symbol<1>], [#impulse.symbol<2>]], regenerate_addresses = [[#impulse.symbol<2>]] }
          : (tensor<1x2xf64>, tensor<f64>, tensor<2xui64>, tensor<f64>, tensor<f64>) -> (tensor<1x2xf64>, tensor<f64>, tensor<i1>, tensor<2xui64>)
      %step2:4 = impulse.mh @test(%step1#3, %mean, %stddev) given %step1#0 weight %step1#1
          { name = "mh_2", selection = [[#impulse.symbol<1>], [#impulse.symbol<2>]], regenerate_addresses = [[#impulse.symbol<1>]] }
          : (tensor<1x2xf64>, tensor<f64>, tensor<2xui64>, tensor<f64>, tensor<f64>) -> (tensor<1x2xf64>, tensor<f64>, tensor<i1>, tensor<2xui64>)
...
// CHECK-NEXT: %[[R_RS1:.+]] = impulse.reshape %[[R_REGEN]]#1 : (tensor<f64>) -> tensor<1x1xf64>
// CHECK-NEXT: %[[R_TR1:.+]] = impulse.dynamic_update_slice %[[R_TRACE_INIT]], %[[R_RS1]], %[[R_C0]], %[[R_C0]] : (tensor<1x2xf64>, tensor<1x1xf64>, tensor<i64>, tensor<i64>) -> tensor<1x2xf64>
// CHECK-NEXT: %[[R_KEPT_SLICED:.+]] = impulse.slice %[[R_ARG0]] {limit_indices = array<i64: 1, 2>, start_indices = array<i64: 0, 1>, strides = array<i64: 1, 1>} : (tensor<1x2xf64>) -> tensor<1x1xf64>
// CHECK-NEXT: %[[R_KEPT:.+]] = impulse.reshape %[[R_KEPT_SLICED]] : (tensor<1x1xf64>) -> tensor<f64>
...
// CHECK-NEXT: %[[R_RS2:.+]] = impulse.reshape %[[R_KEPT]] : (tensor<f64>) -> tensor<1x1xf64>
// CHECK-NEXT: %[[R_TR2:.+]] = impulse.dynamic_update_slice %[[R_TR1]], %[[R_RS2]], %[[R_C0]], %[[R_C1]] : (tensor<1x2xf64>, tensor<1x1xf64>, tensor<i64>, tensor<i64>) -> tensor<1x2xf64>
...
// CHECK: %[[R0_KEPT_SLICED:.+]] = impulse.slice %[[R0_ARG0]] {limit_indices = array<i64: 1, 1>, start_indices = array<i64: 0, 0>, strides = array<i64: 1, 1>} : (tensor<1x2xf64>) -> tensor<1x1xf64>
// CHECK-NEXT: %[[R0_KEPT:.+]] = impulse.reshape %[[R0_KEPT_SLICED]] : (tensor<1x1xf64>) -> tensor<f64>
...
// CHECK-NEXT: %[[R0_RS1:.+]] = impulse.reshape %[[R0_KEPT]] : (tensor<f64>) -> tensor<1x1xf64>
// CHECK-NEXT: %[[R0_TR1:.+]] = impulse.dynamic_update_slice %[[R0_TRACE_INIT]], %[[R0_RS1]], %[[R0_C0]], %[[R0_C0]] : (tensor<1x2xf64>, tensor<1x1xf64>, tensor<i64>, tensor<i64>) -> tensor<1x2xf64>
...
// CHECK-NEXT: %[[R0_RS2:.+]] = impulse.reshape %[[R0_REGEN]]#1 : (tensor<f64>) -> tensor<1x1xf64>
// CHECK-NEXT: %[[R0_TR2:.+]] = impulse.dynamic_update_slice %[[R0_TR1]], %[[R0_RS2]], %[[R0_C0]], %[[R0_C1]] : (tensor<1x2xf64>, tensor<1x1xf64>, tensor<i64>, tensor<i64>) -> tensor<1x2xf64>
