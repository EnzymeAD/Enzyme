; RUN: if [ %llvmver -ge 13 ]; then %opt < %s %loadEnzyme -print-activity-analysis -activity-analysis-func=ad_mp_square_ -o /dev/null | FileCheck %s; fi
...
; CHECK: float* %X: icv:0
; CHECK-NEXT: alloca_1
; CHECK-NEXT:   %"ad_mp_square_$X_PTR" = alloca float*, align 8: icv:0 ici:1
; CHECK-NEXT:   store float* null, float** %"ad_mp_square_$X_PTR", align 8, !tbaa !0: icv:1 ici:1
; CHECK-NEXT:   %"(i8**)ad_mp_square_$X_PTR$" = bitcast float** %"ad_mp_square_$X_PTR" to i8**: icv:0 ici:1
; CHECK-NEXT:   %func_result = call i32 @for_alloc_allocatable(i64 4, i8** nonnull %"(i8**)ad_mp_square_$X_PTR$", i32 262144): icv:1 ici:0
; CHECK-NEXT:   %X_fetch.1 = load float, float* %X, align 1, !tbaa !5: icv:0 ici:0
; CHECK-NEXT:   %"ad_mp_square_$X_PTR_fetch.2" = load float*, float** %"ad_mp_square_$X_PTR", align 8, !tbaa !0: icv:0 ici:1
; CHECK-NEXT:   %rel.not = icmp eq float* %"ad_mp_square_$X_PTR_fetch.2", null: icv:1 ici:1
; CHECK-NEXT:   br i1 %rel.not, label %LHS_not_allocated_lab8, label %LHS_allocated_lab7: icv:1 ici:1
; CHECK-NEXT: LHS_not_allocated_lab8
; CHECK-NEXT:   %func_result4 = call i32 @for_alloc_allocatable_handle(i64 4, i8** nonnull %"(i8**)ad_mp_square_$X_PTR$", i32 262144, i8* null): icv:1 ici:0
; CHECK-NEXT:   %"ad_mp_square_$X_PTR_fetch.3.pre" = load float*, float** %"ad_mp_square_$X_PTR", align 8, !tbaa !0: icv:0 ici:1
; CHECK-NEXT:   br label %LHS_allocated_lab7: icv:1 ici:1
; CHECK-NEXT: LHS_allocated_lab7
; CHECK-NEXT:   %"ad_mp_square_$X_PTR_fetch.3" = phi float* [ %"ad_mp_square_$X_PTR_fetch.3.pre", %LHS_not_allocated_lab8 ], [ %"ad_mp_square_$X_PTR_fetch.2", %alloca_1 ]: icv:0 ici:1
; CHECK-NEXT:   store float %X_fetch.1, float* %"ad_mp_square_$X_PTR_fetch.3", align 8, !tbaa !7: icv:1 ici:0
; CHECK-NEXT:   %"ad_mp_square_$X_PTR_fetch.3_fetch.1" = load float, float* %"ad_mp_square_$X_PTR_fetch.3", align 1, !tbaa !5: icv:0 ici:0
; CHECK-NEXT:   %mul = fmul reassoc ninf nsz arcp contract afn float %"ad_mp_square_$X_PTR_fetch.3_fetch.1", %"ad_mp_square_$X_PTR_fetch.3_fetch.1": icv:0 ici:0
; CHECK-NEXT:   %"(i8*)ad_mp_square_$X_PTR_fetch.8$" = bitcast float* %"ad_mp_square_$X_PTR_fetch.3" to i8*: icv:0 ici:1
; CHECK-NEXT:   %func_result6 = tail call i32 @for_dealloc_allocatable(i8* nonnull %"(i8*)ad_mp_square_$X_PTR_fetch.8$", i32 262144): icv:1 ici:1
; CHECK-NEXT:   %rel8 = icmp eq i32 %func_result6, 0: icv:1 ici:1
; CHECK-NEXT:   br i1 %rel8, label %dealloc.list.end16, label %dealloc.list.then15: icv:1 ici:1
; CHECK-NEXT: dealloc.list.then15
; CHECK-NEXT:   %func_result12 = tail call i32 @for_dealloc_allocatable(i8* nonnull %"(i8*)ad_mp_square_$X_PTR_fetch.8$", i32 262144): icv:1 ici:1
; CHECK-NEXT:   br label %dealloc.list.end16: icv:1 ici:1
; CHECK-NEXT: dealloc.list.end16
; CHECK-NEXT:   ret float %mul: icv:1 ici:1