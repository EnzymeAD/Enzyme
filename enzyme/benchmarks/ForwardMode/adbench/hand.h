...
void hand_objective_b(const double *theta, double *thetad, int bone_count,
                      const char **bone_names, const int *parents,
                      Matrix *base_relatives, Matrix *inverse_base_absolutes,
                      Matrix *base_positions, Matrix *weights,
                      const Triangle * /* TFIX */
...
void read_hand_instance(const string &model_dir, const string &fn_in,
                        vector<double> *theta, HandDataLightMatrix *data,
                        vector<double> *us = nullptr) {
...
typedef void (*deriv_obj_t)(double const *theta, double *dtheta, int bone_count,
                            const char **bone_names, const int *parents,
                            Matrix *base_relatives,
                            Matrix *inverse_base_absolutes,
                            Matrix *base_positions, Matrix *weights,
...
void calculate_jacobian_simple(HandObjectiveData *objective_input,
                               struct HandInput &input,
                               struct HandOutput &result,
                               std::vector<double> &theta_d,
                               std::vector<double> &us_d,
                               std::vector<double> &us_jacobian_column) {
  for (int i = 0; i < theta_d.size(); i++) {
    ...
    theta_d[i] = 1.0;
    deriv_obj(input.theta.data(), theta_d.data(), objective_input->bone_count,
              objective_input->bone_names, objective_input->parents,
              objective_input->base_relatives,
              objective_input->inverse_base_absolutes,
              &objective_input->base_positions, &objective_input->weights,
              objective_input->triangles, objective_input->is_mirrored,
              objective_input->corresp_count, objective_input->correspondences,
              &objective_input->points, result.objective.data(),
...
template <>
void calculate_jacobian_simple<dhand_objective>(
    HandObjectiveData *objective_input, struct HandInput &input,
    struct HandOutput &result, std::vector<double> &theta_d,
    std::vector<double> &us_d, std::vector<double> &us_jacobian_column) {
  ...
  dhand_objective(
      input.theta.data(), result.jacobian.data(), objective_input->bone_count,
      objective_input->bone_names, objective_input->parents,
      objective_input->base_relatives, objective_input->inverse_base_absolutes,
      &objective_input->base_positions, &objective_input->weights,
      objective_input->triangles, objective_input->is_mirrored,
      objective_input->corresp_count, objective_input->correspondences,
      &objective_input->points, result.objective.data(), err.data());
}

template <>
void calculate_jacobian_simple<hand_objective_d>(
    HandObjectiveData *objective_input, struct HandInput &input,
    struct HandOutput &result, std::vector<double> &theta_d,
    std::vector<double> &us_d, std::vector<double> &us_jacobian_column) {
...
void calculate_jacobian_complicated(HandObjectiveData *objective_input,
                                    struct HandInput &input,
                                    struct HandOutput &result,
                                    std::vector<double> &theta_d,
                                    std::vector<double> &us_d,
  ...
  for (int i = 0; i < theta_d.size(); i++) {
    ...
    deriv_obj_complicated(
        input.theta.data(), theta_d.data(), input.us.data(), us_d.data(),
        objective_input->bone_count, objective_input->bone_names,
        objective_input->parents, objective_input->base_relatives,
        objective_input->inverse_base_absolutes,
        &objective_input->base_positions, &objective_input->weights,
        objective_input->triangles, objective_input->is_mirrored,
        objective_input->corresp_count, objective_input->correspondences,
  ...
  for (int i = 0; i < us_d.size(); i++) {
    ...
    if (i % 2 == 0) {
      ...
      for (int j = 0; j < 3; j++) {
        result.jacobian[nrows + 3 * ((i - 1) / 2) + j] =
...
void calculate_jacobian(HandObjectiveData *objective_input,
                        struct HandInput &input, struct HandOutput &result,
                        bool complicated, std::vector<double> &theta_d,
                        std::vector<double> &us_d,
                        std::vector<double> &us_jacobian_column) {
  if (complicated) {
    calculate_jacobian_complicated<deriv_obj_complicated>(
        objective_input, input, result, theta_d, us_d, us_jacobian_column);
  } else {
    calculate_jacobian_simple<deriv_obj>(objective_input, input, result,
                                         theta_d, us_d, us_jacobian_column);
  }
}
...
HandObjectiveData *convert_to_hand_objective_data(const HandInput &input) {
  ...
  for (int i = 0; i < result->bone_count; i++) {
    ...
    result->inverse_base_absolutes[i] =
...
int main(const int argc, const char *argv[]) {
  ...
  for (auto path : paths) {
...

    {
      ...
      struct HandOutput result = {std::vector<double>(err_size), ncols,
                                  err_size,
...

      {
        ...
        calculate_jacobian<hand_objective_d, hand_objective_complicated_d>(
            objective_input, input, result, params.is_complicated, theta_d,
            us_d, us_jacobian_column);
...

    {
...

      {
        struct timeval start, end;
        gettimeofday(&start, NULL);
        calculate_jacobian<dhand_objective, dhand_objective_complicated>(
            objective_input, input, result, params.is_complicated, theta_d,
            us_d, us_jacobian_column);
        gettimeofday(&end, NULL);
