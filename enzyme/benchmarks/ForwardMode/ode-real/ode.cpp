...
template <typename T> T brusselator_f(T x, T y, T t) {
...
brusselator_2d_loop(double *__restrict du, double *__restrict dv,
                    const double *__restrict u, const double *__restrict v,
                    const double *__restrict p, double t) {
  double A = p[0];
  double B = p[1];
  double alpha = p[2];
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      GETnb(du, i, j) =
          alpha * (GET(u, im1, j) + GET(u, ip1, j) + GET(u, i, jp1) +
                   GET(u, i, jm1) - 4 * GET(u, i, j)) +
          B + u2v - (A + 1) * GET(u, i, j) + brusselator_f(x, y, t);

      GETnb(dv, i, j) =
          alpha * (GET(v, im1, j) + GET(v, ip1, j) + GET(v, i, jp1) +
                   GET(v, i, jm1) - 4 * GET(v, i, j)) +
          A * GET(u, i, j) - u2v;
    }
  }
}
...
void lorenz(const state_type &x, state_type &dxdt, double t) {
  ...
  brusselator_2d_loop(dxdt.c_array(), dxdt.c_array() + N * N, x.data(),
                      x.data() + N * N, p, t);
}

// init_brusselator(x.c_array(), x.c_array() + N*N)

double foobar(const double *p, const state_type x, const state_type adjoint,
              double t) {
  double dp[3] = {0.};
  ...
  __enzyme_autodiff<void>(
      brusselator_2d_loop,
      //                            enzyme_dup, dxdu.c_array(),
      //                            dadjoint_inp.c_array(), enzyme_dup,
      //                            dxdu.c_array() + N * N,
      //                            dadjoint_inp.c_array() + N * N,
      enzyme_dupnoneed, nullptr, dadjoint_inp.data(), enzyme_dupnoneed, nullptr,
      dadjoint_inp.data() + N * N, enzyme_dup, x.data(), dx.data(), enzyme_dup,
      x.data() + N * N, dx.data() + N * N, enzyme_dup, p, dp, enzyme_const, t);
...
void abrusselator_2d_loop(aVector &du, aVector &dv, aVector &u, aVector &v,
                          aVector &p, double t) {
  adouble A = p(0);
  adouble B = p(1);
  adouble alpha = p(2);
...
double afoobar(const double *p_in, const state_type x, const state_type adjoint,
               double t) {
  adept::Stack stack;
...
*/
void get_b(const double *x, double *xb, unsigned int i, unsigned int j,
           double getb) {
  double get;
...
void brusselator_2d_loop_b(double *du, double *dub, double *dv, double *dvb,
                           const double *u, double *ub, const double *v,
                           double *vb, const double *p, double *pb, double t) {
  double A = p[0];
  double Ab = 0.0;
  double B = p[1];
  double Bb = 0.0;
  ...
  *vb = 0.0;
  ...
  for (int i = N - 1; i > -1; --i)
    for (int j = N - 1; j > -1; --j) {
      ...
      temp = u[N * i + j];
      alphab = alphab +
               (v[N * im1 + j] + v[N * ip1 + j] + v[N * i + jp1] +
                v[N * i + jm1] - 4 * v[N * i + j]) *
                   dvb[N * i + j] +
               (u[N * im1 + j] + u[N * ip1 + j] + u[N * i + jp1] +
                u[N * i + jm1] - 4 * u[N * i + j]) *
                   dub[N * i + j];
      tempb = alpha * dvb[N * i + j];
      Ab = Ab + u[N * i + j] * dvb[N * i + j] - u[N * i + j] * dub[N * i + j];
      ub[N * i + j] =
          ub[N * i + j] + A * dvb[N * i + j] - (A + 1) * dub[N * i + j];
      ...
      ub[N * i + j] =
          ub[N * i + j] + 2 * temp * v[N * i + j] * u2vb - 4 * tempb;
    }
  alphab = alphab / (dx * dx);
  pb[2] = pb[2] + alphab;
  pb[1] = pb[1] + Bb;
...
#else
/*
  Differentiation of brusselator_2d_loop in reverse (adjoint) mode (with options
  i4 dr8 r4): gradient     of useful results: *du *dv with respect to varying
  inputs: *p *u *du *v *dv RW status of diff variables: *p:out *u:out *du:in-out
  *v:out *dv:in-out Plus diff mem management of: p:in u:in du:in v:in dv:in
*/
void brusselator_2d_loop_b(double *du, double *dub, double *dv, double *dvb,
                           const double *u, double *ub, const double *v,
                           double *vb, const double *p, double *pb, double t) {
  ...
  *vb = 0.0;
  ...
  for (int i = N - 1; i > -1; --i)
    for (int j = N - 1; j > -1; --j) {
      ...
      popInteger4((int *)&jp1);
      popInteger4((int *)&jm1);
      alphab = alphab + (result1 + result2 + result3 + result4 - 4 * result5) *
                            dvb[N * i + j];
      tempb = alpha * dvb[N * i + j];
...
double tfoobar(const double *p, const state_type x, const state_type adjoint,
               double t) {
  double dp[3] = {0.};
  ...
  brusselator_2d_loop_b(nullptr, dadjoint_inp.data(), nullptr,
                        dadjoint_inp.data() + N * N, x.data(), dx.data(),
                        x.data() + N * N, dx.data() + N * N, p, dp, t);
