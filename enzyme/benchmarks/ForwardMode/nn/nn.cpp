 ...
 * large exponents.
 */
void aneural_network_softmax(
    adept::FixedArray<float, true, MNIST_LABELS> &activations, int length) {
  int i;
  aReal sum, max;
...
static void makeexps(float *exps, const float *activations, int length,
                     double max) {
  for (int i = 0; i < length; i++) {
 ...
 * large exponents.
 */
static void neural_network_softmax_v2(const float *activations, float *outp,
                                      int length) {
  int i;
  float sum, max;
 ...
 */
void neural_network_hypothesis(const mnist_image_t *image,
 ...
 * network and calculate the activations.
 */
static float neural_network_hypothesis_v2(const mnist_image_t *image,
                                          const neural_network_t *network,
                                          uint8_t label) {
...
static aReal neural_network_hypothesis_adept(const mnist_image_t *image,
                                             const aneural_network_t *network,
                                             uint8_t label) {
...
static void calculateDerivatives_adept(mnist_image_t *image, bool run,
                                       adept::Stack &stack,
                                       const aneural_network_t *anetwork,
                                       neural_network_t *gradient,
                                       uint8_t label) {
...
void neural_network_softmax_b(float *activations, float *activationsb,
                              int length) {
  float sum, max;
  float sumb, maxb;
  int branch;
...
/*
  Differentiation of neural_network_hypothesis_tapenadesource in reverse
  (adjoint) mode: gradient     of useful results:
  neural_network_hypothesis_tapenadesource *network.b[0:10-1]
  *network.W[0:10-1][0:28*28-1] with respect to varying inputs:
  *network.b[0:10-1] *network.W[0:10-1][0:28*28-1] RW status of diff variables:
  neural_network_hypothesis_tapenadesource:in-killed *network.b[0:10-1]:incr
  *network.W[0:10-1][0:28*28-1]:incr Plus diff mem management of: network:in
  *network.b:in *network.W:in *network.W[0:10-1]:in
*/
static void neural_network_hypothesis_tapenadesource_b(
    const mnist_image_t *image, const neural_network_t *network,
    neural_network_t *networkb, uint8_t label,
    float neural_network_hypothesis_tapenadesourceb) {
  ...
  for (int i = 9; i > -1; --i) {
    for (int j = 783; j > -1; --j)
      networkb->W[i][j] = networkb->W[i][j] +
                          (float)image->pixels[j] * activationsb[i] / 255.0f;
...
static void calculateDerivatives(mnist_image_t *image,
                                 const neural_network_t *network,
                                 neural_network_t *gradient, uint8_t label) {
  __enzyme_autodiff<void>(neural_network_hypothesis_v2, enzyme_const, image,
                          network, gradient, enzyme_const, label);
}
...
float neural_network_training_step(mnist_dataset_t *dataset,
  ...
  for (i = 0, total_loss = 0; i < dataset->size; i++) {
    ...
    // calculateDerivatives_adept(image, i != 0, stack, &anetwork, &gradient,
    // label); neural_network_hypothesis_tapenadesource_b(image, network,
    // &gradient, label, 1.0);

    total_loss += neural_network_gradient_update(
        image, network, (neural_network_gradient_t *)&gradient, label);

    // total_loss +=neural_network_gradient_update(image, network,
    // (neural_network_gradient_t*)&gradient2, label);
  ...
  for (i = 0; i < MNIST_LABELS; i++) {
    ...
    for (j = 0; j < MNIST_IMAGE_SIZE + 1; j++) {
      network->W[i][j] -=
          learning_rate * gradient.W[i][j] / ((float)dataset->size);
    }
  }
 ...
 * Run one step of gradient descent and update the neural network.
 */
float neural_network_training_step_enzyme(mnist_dataset_t *dataset,
                                          neural_network_t *network,
                                          float learning_rate) {
  neural_network_t gradient = {0};
  neural_network_t gradient2 = {0};
 ...
 * Run one step of gradient descent and update the neural network.
 */
float neural_network_training_step_adept(mnist_dataset_t *dataset,
                                         neural_network_t *network,
                                         float learning_rate) {
  neural_network_t gradient = {0};
  neural_network_t gradient2 = {0};
 ...
 * Run one step of gradient descent and update the neural network.
 */
float neural_network_training_step_tapenade(mnist_dataset_t *dataset,
                                            neural_network_t *network,
                                            float learning_rate) {
  neural_network_t gradient = {0};
  neural_network_t gradient2 = {0};
  ...
  for (i = 0, total_loss = 0; i < dataset->size; i++) {
    ...
    neural_network_hypothesis_tapenadesource_b(image, network, &gradient, label,
                                               1.0);
    // total_loss +=neural_network_gradient_update(image, network,
    // (neural_network_gradient_t*)&gradient2, label);
