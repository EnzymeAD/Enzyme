...
/*!\brief Create a templated FFT/Inverse FFT object
  ...
  gfft.ifft(data);
*/
template <unsigned P, typename T = double> class FFT {
  enum { N = 1 << P };
...
template <unsigned N> class DanielsonLanczos<N, adouble> {
...
template <> class DanielsonLanczos<1, adouble> {
public:
  void apply(aVector data, int iSign) {}
};

template <unsigned P> class FFT<P, adouble> {
  enum { N = 1 << P };
