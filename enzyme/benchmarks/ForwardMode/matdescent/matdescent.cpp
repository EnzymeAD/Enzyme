...
*/
void matvec_real_b(double *mat, double *matb, double *vec,
                   double matvec_realb) {
  double *out;
  double *outb;
  int ii1;
...
static adouble matvec(aMatrix &mat, Vector &vec) {
...
static void tapenade_sincos(double *Min, double *Mout, double *Vin,
                            double *Vout) {
...

  {
    ...
    gettimeofday(&end, NULL);
    printf("tapenade %0.6f res'=%f %f %f\n", tdiff(&start, &end), Mout[1],
           Mout[2], Mout[3]);
  }
}
static void enzyme_sincos(double *Min, double *Mout, double *Vin,
                          double *Vout) {
