...
void calculate_reproj_error_jacobian_part_reverse(struct BAInput &input,
  ...
  double err[2]; // stores fictive result
                 // (Tapenade doesn't calculate an original function in reverse
                 // mode)
  ...
  for (int i = 0; i < input.p; i++) {
    ...
    for (auto &a : reproj_err_d_row)
      a = 0.0;

    deriv_reproj(&input.cams[camIdx * BA_NCAMPARAMS], cam_gradient_part,
                 &input.X[ptIdx * 3], x_gradient_part, &input.w[i],
                 weight_gradient_part, &input.feats[i * 2], err, errb);
