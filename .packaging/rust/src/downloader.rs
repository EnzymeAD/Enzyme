...
pub fn download_tarball(repo: PathBuf, download_filename: PathBuf) -> Result<(), String> {
    if std::path::Path::new(&download_filename).exists() {
        match std::fs::remove_file(&download_filename) {
            Ok(()) => {},
            Err(e) => return Err(format!("unable to delete file, download location blocked: {}",e)),
        }
    }

    let mut file = match OpenOptions::new().write(true).read(true).create(true).open(&download_filename) {
    ...
    match file.write_all(&downloaded_data) {
      Ok(_) => {},
      Err(e) => return Err(format!("Unable to write download {} to file {}. {}", repo.display(), download_filename.display(), e)),
    };
...
pub fn download(to_download: &str) -> Result<(), String> {
    ...
    let repo = match to_download {
        ...
        _        => return Err("Unknown download argument. Try enzyme xor rustc (includes llvm and others).".to_owned()),
    };
    ...
    let download_filename = utils::get_local_download_dir().join(to_download.to_owned() + ".tar.gz");
    ...
    */
    ...
    match unpack(download_filename.to_str().unwrap(), dst_dir.to_str().unwrap()) {
      Ok(_) => {},
