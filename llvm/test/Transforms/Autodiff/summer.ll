; RUN: opt < %s -load=%llvmshlibdir/LLVMEnzyme%shlibext -enzyme -inline -ipconstprop -deadargelim -mem2reg -instsimplify -adce -loop-deletion -correlated-propagation -simplifycfg -S | FileCheck %s
...
entry:
  %0 = tail call double (double (double*, i64)*, ...) @llvm.autodiff.p0f_f64p0f64i64f(double (double*, i64)* nonnull @summer, double* %x, double* %xp, i64 %n)
  ret void
}
...
declare double @llvm.autodiff.p0f_f64p0f64i64f(double (double*, i64)*, ...) #1
