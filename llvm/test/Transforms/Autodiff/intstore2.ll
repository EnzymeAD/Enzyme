; RUN: opt < %s -load=%llvmshlibdir/LLVMEnzyme%shlibext -enzyme -mem2reg -instsimplify -simplifycfg -early-cse -adce -S | FileCheck %s
...
entry:
  %0 = tail call double (void (double**, double**)*, ...) @llvm.autodiff.p0f_isVoidp0p0f64p0p0f64f(void (double**, double**)* nonnull @store, double** %x, double** %xp, double** %y, double** %yp)
  ret void
}

; Function Attrs: nounwind
declare double @llvm.autodiff.p0f_isVoidp0p0f64p0p0f64f(void (double**, double**)*, ...) #2
