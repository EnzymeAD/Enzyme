...
entry:
  %0 = tail call double (double (double, i32)*, ...) @llvm.autodiff.p0f_f64f64i32f(double (double, i32)* nonnull @function, double %x, i32 %n)
  ret double %0
}

; Function Attrs: nounwind
declare double @llvm.autodiff.p0f_f64f64i32f(double (double, i32)*, ...) #5
...
; CHECK-NEXT:  %3 = call fastcc i8* @augmented_allocateAndSet.1(double** nonnull %array.i, double** nonnull %"array'ipa.i", double %x, i32 %n) #6
...
; CHECK-NEXT:  %4 = call fastcc double @diffeallocateAndSet(double** nonnull %array.i, double %x, i32 %n, { i8* } %[[oldret]]) #6
...
; CHECK: define internal fastcc i8* @augmented_allocateAndSet.1(double** nocapture %arrayp, double** nocapture %"arrayp'", double %x, i32 %n) unnamed_addr #0 {
; CHECK-NEXT: entry:
...
; CHECK: define internal fastcc double @diffeallocateAndSet(double** nocapture %arrayp, double %x, i32 %n, { i8* } %tapeArg) unnamed_addr #0 {
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = extractvalue { i8* } %tapeArg, 0
; CHECK-NEXT:   %conv = zext i32 %n to i64
; CHECK-NEXT:   %mul = shl nuw nsw i64 %conv, 3
; CHECK-NEXT:   %call = tail call i8* @malloc(i64 %mul)
; CHECK-NEXT:   %1 = bitcast double** %arrayp to i8**
; CHECK-NEXT:   store i8* %call, i8** %1, align 8, !tbaa !2
; CHECK-NEXT:   %arrayidx = getelementptr inbounds i8, i8* %call, i64 24
; CHECK-NEXT:   %2 = bitcast i8* %arrayidx to double*
; CHECK-NEXT:   store double %x, double* %2, align 8, !tbaa !6
; CHECK-NEXT:   %"arrayidx'ipg" = getelementptr i8, i8* %0, i64 24
; CHECK-NEXT:   %"'ipc" = bitcast i8* %"arrayidx'ipg" to double*
; CHECK-NEXT:   %3 = load double, double* %"'ipc", align 8
