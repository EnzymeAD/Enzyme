...
entry:
  %0 = tail call double (double (double)*, ...) @llvm.autodiff.p0f_f64f64f(double (double)* nonnull @mulglobal, double %x)
  ret double %0
}

; Function Attrs: nounwind
declare double @llvm.autodiff.p0f_f64f64f(double (double)*, ...) #2
...
; CHECK: define internal { double } @diffemulglobal(double %x, double %[[differet:.+]])
; CHECK-NEXT: entry:
